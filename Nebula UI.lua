...
Probably one of my best code optimized UI Library
...
local Library = {
	Themes = {
		...
		Blood = { Main = Color3.fromRGB(25, 15, 15), Sidebar = Color3.fromRGB(18, 10, 10), Element = Color3.fromRGB(40, 20, 20), Accent = Color3.fromRGB(220, 40, 40), Text = Color3.fromRGB(255, 230, 230), TextDark = Color3.fromRGB(160, 100, 100) },
		Ocean = { Main = Color3.fromRGB(15, 25, 35), Sidebar = Color3.fromRGB(10, 18, 25), Element = Color3.fromRGB(25, 40, 55), Accent = Color3.fromRGB(40, 180, 255), Text = Color3.fromRGB(230, 245, 255), TextDark = Color3.fromRGB(120, 150, 180) },
		Grape = { Main = Color3.fromRGB(28, 18, 35), Sidebar = Color3.fromRGB(20, 12, 25), Element = Color3.fromRGB(45, 28, 55), Accent = Color3.fromRGB(180, 80, 255), Text = Color3.fromRGB(250, 230, 255), TextDark = Color3.fromRGB(160, 130, 180) },
		Obsidian = { Main = Color3.fromRGB(10, 10, 10), Sidebar = Color3.fromRGB(5, 5, 5), Element = Color3.fromRGB(20, 20, 20), Accent = Color3.fromRGB(255, 255, 255), Text = Color3.fromRGB(255, 255, 255), TextDark = Color3.fromRGB(120, 120, 120) }
	},
...
function Library:CreateWindow(options)
	...
	local uiPos = type(options.Position) == "userdata" and options.Position or UDim2.new(0.5, 0, 0.5, 0)
	...
	local MainFrame = Utility:Create("Frame", { Size = uiSize, Position = uiPos, AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = self.Themes[self.CurrentTheme].Main, Parent = NebulaGui, ClipsDescendants = true, Visible = false })
	local MainScale = Utility:Create("UIScale", { Scale = 1, Parent = MainFrame })
	...
	local TitleLabel = Utility:Create("TextLabel", { Size = UDim2.new(0.8, 0, 0, 40), Position = UDim2.new(0.1, 0, 0, 5), BackgroundTransparency = 1, Text = title, TextColor3 = self.Themes[self.CurrentTheme].Text, Font = Enum.Font.GothamBold, Parent = Sidebar, TextScaled = true })
	...
	local MinIcon = Utility:Create("TextButton", { Size = UDim2.new(0, 0, 0, 0), Position = UDim2.new(0, 20, 0, 20), BackgroundColor3 = self.Themes[self.CurrentTheme].Main, Text = "N", TextColor3 = self.Themes[self.CurrentTheme].Accent, Font = Enum.Font.GothamBold, TextSize = 18, Parent = NebulaGui, Visible = false })
	...
	MinBtn.MouseButton1Click:Connect(function()
		local tw = Utility:Tween(MainScale, {Scale = 0}, 0.3)
	...
	MinIcon.MouseButton1Click:Connect(function()
		...
		tw.Completed:Connect(function()
			...
			Utility:Tween(MainScale, {Scale = 1}, 0.3)
		end)
	end)

	local PromptOverlay = Utility:Create("Frame", { Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0.5, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.fromRGB(0, 0, 0), BackgroundTransparency = 1, Parent = MainFrame, Visible = false, ZIndex = 50 })
	...
	PromptYes.MouseButton1Click:Connect(function()
		Utility:Tween(PromptOverlay, {BackgroundTransparency = 1}, 0.2)
		local tw = Utility:Tween(PromptBox, {Size = UDim2.new(0, 0, 0, 0)}, 0.2)
		tw.Completed:Connect(function() Library:Destroy() end)
	end)
	...
	task.spawn(function()
		local function OpenMainUI()
			MainFrame.Visible = true
			MainScale.Scale = 0
			Utility:Tween(MainScale, {Scale = 1}, 0.4)
			Library:Load()
		end
		...
		local assetUrl = Utility:GetImage({Name = "SBEUILIB", Url = "https://i.ibb.co/xK20Pwph/SBEUILib.png"})
		...
		if options.KeySystem and options.KeySettings then
			...
			local KeyCloseBtn = Utility:Create("TextButton", { Size = UDim2.new(0, 30, 0, 30), Position = UDim2.new(1, -35, 0, 5), BackgroundTransparency = 1, Text = "X", TextColor3 = Library.Themes[Library.CurrentTheme].TextDark, Font = Enum.Font.GothamBold, TextSize = 14, Parent = KeyFrame })
			Library:RegisterTheme(KeyCloseBtn, "TextColor3", "TextDark")

			local KeyPromptOverlay = Utility:Create("Frame", { Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0.5, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.fromRGB(0, 0, 0), BackgroundTransparency = 1, Parent = KeyFrame, Visible = false, ZIndex = 60 })
			Utility:Create("UICorner", { CornerRadius = UDim.new(0, 8), Parent = KeyPromptOverlay })

			local KeyPromptBox = Utility:Create("Frame", { Size = UDim2.new(0, 0, 0, 0), Position = UDim2.new(0.5, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Library.Themes[Library.CurrentTheme].Main, ClipsDescendants = true, Parent = KeyPromptOverlay, ZIndex = 61 })
			Utility:Create("UICorner", { CornerRadius = UDim.new(0, 8), Parent = KeyPromptBox })
			Library:RegisterTheme(KeyPromptBox, "BackgroundColor3", "Main")

			local KeyPromptLbl = Utility:Create("TextLabel", { Size = UDim2.new(0, 250, 0, 60), Position = UDim2.new(0.5, -125, 0, 0), BackgroundTransparency = 1, Text = "Close Key System?", TextColor3 = Library.Themes[Library.CurrentTheme].Text, Font = Enum.Font.GothamSemibold, TextSize = 14, Parent = KeyPromptBox, ZIndex = 62 })
			Library:RegisterTheme(KeyPromptLbl, "TextColor3", "Text")

			local KeyPromptYes = Utility:Create("TextButton", { Size = UDim2.new(0, 100, 0, 30), Position = UDim2.new(0.5, -110, 0, 70), BackgroundColor3 = Library.Themes[Library.CurrentTheme].Accent, Text = "Yes", TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.GothamBold, TextSize = 13, Parent = KeyPromptBox, ZIndex = 62 })
			Utility:Create("UICorner", { CornerRadius = UDim.new(0, 4), Parent = KeyPromptYes })
			Library:RegisterTheme(KeyPromptYes, "BackgroundColor3", "Accent")

			local KeyPromptNo = Utility:Create("TextButton", { Size = UDim2.new(0, 100, 0, 30), Position = UDim2.new(0.5, 10, 0, 70), BackgroundColor3 = Library.Themes[Library.CurrentTheme].Element, Text = "No", TextColor3 = Library.Themes[Library.CurrentTheme].Text, Font = Enum.Font.GothamBold, TextSize = 13, Parent = KeyPromptBox, ZIndex = 62 })
			Utility:Create("UICorner", { CornerRadius = UDim.new(0, 4), Parent = KeyPromptNo })
			Library:RegisterTheme(KeyPromptNo, "BackgroundColor3", "Element")
			Library:RegisterTheme(KeyPromptNo, "TextColor3", "Text")

			KeyCloseBtn.MouseButton1Click:Connect(function()
				KeyPromptOverlay.Visible = true
				Utility:Tween(KeyPromptOverlay, {BackgroundTransparency = 0.5}, 0.2)
				Utility:Tween(KeyPromptBox, {Size = UDim2.new(0, 250, 0, 120)}, 0.2)
			end)

			KeyPromptNo.MouseButton1Click:Connect(function()
				Utility:Tween(KeyPromptOverlay, {BackgroundTransparency = 1}, 0.2)
				local tw = Utility:Tween(KeyPromptBox, {Size = UDim2.new(0, 0, 0, 0)}, 0.2)
				tw.Completed:Connect(function() KeyPromptOverlay.Visible = false end)
			end)

			KeyPromptYes.MouseButton1Click:Connect(function()
				Utility:Tween(KeyPromptOverlay, {BackgroundTransparency = 1}, 0.2)
				local tw = Utility:Tween(KeyPromptBox, {Size = UDim2.new(0, 0, 0, 0)}, 0.2)
				tw.Completed:Connect(function() Library:Destroy() end)
			end)
