-- Orbital.lua

--[[

Oribital UI /
SugaBlaz's Executor UI Library (Main Version)

Made with passion by SugaDev

SugaDev      | Programming + Design
AzkaDev      | Design

Version 2.2

Update Log:
	Verison 1:
		- Main UI Framework completed

	Version 1.1:
		- Added Slider
		- Added Toggle
		- Added Sections
		- Added Buttons
		- Added Default Themes
		- Added Labels

	Version 1.2:
		- Added Key System
		- Added Fake Loading Frame
		- Added Notification Function

	Version 1.3:
		- Revamped overall code

	Version 1.4:
		- Revamped Key System due to bugs
		- Added Sections

	Version 1.5:
		- Added Dropdown Frame
		- Added UI Library Intro

	Version 1.6:
		- Added Utility Module
		- Made Key System able to "read" links or strings
			2 Supported link types:
				CloudFlare Worker API
				Static Links: rawgithubusercontent, Pastebin.raw
		- Added Tabs (automatiicaly creates when you make a section)

	Version 1.7:
		- Added Multi Dropdown Frame
		- Added Get, Set functions to Toggles and Sliders
		- Added User Configuration saving!!!

	Version 1.8:
		- Added More Themes
		- Added ChangeTheme Function
		- Added Color Picker

	Version 1.9:
		- Added Textboxes to sliders
		- Added Textboxes to Color Picker
		- Revamped Notfication Function
		- Added Tab Search Bar

	Version 1.91:
		- Added a few more themes

	Version 2:
		- Added a Website to teach how to use this confusing ui library
			
	Version 2.1:
		- Added Set function to MultiDropdowns and Dropdowns, added saving and set function for textboxes.
		- Updated Website
	
	Version 2.2:
		- Added settings (Saves)
		- Addded Setting Size to config
		- Settings behave just like the ui library elements, its just a new place to add settings rather than in the main frame.
		- Added SettingBtnColor to default themes
		- Made tab search bar smaller
		- Added SearchBar Color to default themes
		- Added a few more themes
		- Added a few more things to config
		- Added AddSettingTextox, AddSettingSlider, etc.
		- Updated Website

	Version 2.21:
		- Added Keybind (setting)
		- Added Default settings: Toggle GUI Keybind, Toggle Frame Keybind
		- Updated Website
		
Future Improvments:
	- Add a UI Layout button near the setting button (maybe)

]]

local UILibrary = {}

local TS = game:GetService("TweenService")
local HS = game:GetService("HttpService")
local UIS = game:GetService("UserInputService")
local RS = game:GetService("RunService")
local Players = game:GetService("Players")
local CoreGui = game:GetService("CoreGui")

UILibrary.DefaultThemes = {
		-- 1. Default
		Default = {
			TitleColor = Color3.fromRGB(255, 255, 255),
			CollapseBtnColor = Color3.fromRGB(25, 25, 25),
			SettingBtnColor = Color3.fromRGB(255, 255, 255),
			SearchBarColor = Color3.fromRGB(45, 45, 45),
			ButtonColor = Color3.fromRGB(45, 45, 45),
			ButtonHoverColor = Color3.fromRGB(60, 60, 60),
			ToggleColor = Color3.fromRGB(45, 45, 45), 
			ToggleColorOFF = Color3.fromRGB(200, 60, 60),
			ToggleColorON = Color3.fromRGB(60, 200, 60),
			MainFrameColor = Color3.fromRGB(35, 35, 35),
			SeparatorColor = Color3.fromRGB(70, 70, 70),
			TextBoxColor = Color3.fromRGB(45, 45, 45),
			AccentColor = Color3.fromRGB(0, 120, 215),
			SectionColor = Color3.fromRGB(90, 90, 90),
			LabelColor = Color3.fromRGB(200, 200, 200),
			SliderColor = Color3.fromRGB(70, 70, 70),
			SliderHandleColor = Color3.fromRGB(100, 100, 100),
			UIStrokeColor = Color3.fromRGB(60, 60, 60),
			LoadingBarColor = Color3.fromRGB(0, 155, 255),
			TabColor = Color3.fromRGB(55,55,55),
		},

		-- 2. Light Mode (Serenity)
		Serenity = {
			TitleColor = Color3.fromRGB(40, 40, 40),
			CollapseBtnColor = Color3.fromRGB(220, 220, 220),
			SettingBtnColor = Color3.fromRGB(40, 40, 40),
			SearchBarColor = Color3.fromRGB(235, 235, 235),
			ButtonColor = Color3.fromRGB(240, 240, 240),
			ButtonHoverColor = Color3.fromRGB(210, 210, 210),
			ToggleColor = Color3.fromRGB(240, 240, 240),
			ToggleColorOFF = Color3.fromRGB(200, 100, 100),
			ToggleColorON = Color3.fromRGB(100, 200, 100),
			MainFrameColor = Color3.fromRGB(255, 255, 255),
			SeparatorColor = Color3.fromRGB(200, 200, 200),
			TextBoxColor = Color3.fromRGB(230, 230, 230),
			AccentColor = Color3.fromRGB(0, 120, 215),
			SectionColor = Color3.fromRGB(200, 200, 200),
			LabelColor = Color3.fromRGB(80, 80, 80),
			SliderColor = Color3.fromRGB(220, 220, 220),
			SliderHandleColor = Color3.fromRGB(0, 120, 215),
			UIStrokeColor = Color3.fromRGB(200, 200, 200),
			LoadingBarColor = Color3.fromRGB(0, 120, 215),
			TabColor = Color3.fromRGB(240, 240, 240),
		},

		-- 3. Blood Moon (Red/Black)
		BloodMoon = {
			TitleColor = Color3.fromRGB(255, 200, 200),
			CollapseBtnColor = Color3.fromRGB(20, 10, 10),
			SettingBtnColor = Color3.fromRGB(255, 50, 50),
			SearchBarColor = Color3.fromRGB(30, 10, 10),
			ButtonColor = Color3.fromRGB(40, 20, 20),
			ButtonHoverColor = Color3.fromRGB(60, 30, 30),
			ToggleColor = Color3.fromRGB(40, 20, 20),
			ToggleColorOFF = Color3.fromRGB(80, 20, 20),
			ToggleColorON = Color3.fromRGB(220, 40, 40),
			MainFrameColor = Color3.fromRGB(15, 5, 5),
			SeparatorColor = Color3.fromRGB(80, 40, 40),
			TextBoxColor = Color3.fromRGB(30, 15, 15),
			AccentColor = Color3.fromRGB(255, 50, 50),
			SectionColor = Color3.fromRGB(100, 50, 50),
			LabelColor = Color3.fromRGB(200, 160, 160),
			SliderColor = Color3.fromRGB(60, 30, 30),
			SliderHandleColor = Color3.fromRGB(255, 50, 50),
			UIStrokeColor = Color3.fromRGB(100, 30, 30),
			LoadingBarColor = Color3.fromRGB(255, 50, 50),
			TabColor = Color3.fromRGB(50, 25, 25),
		},

		-- 4. Terminal (Green/Black Matrix)
		Terminal = {
			TitleColor = Color3.fromRGB(0, 255, 0),
			CollapseBtnColor = Color3.fromRGB(5, 15, 5),
			SettingBtnColor = Color3.fromRGB(0, 255, 0),
			SearchBarColor = Color3.fromRGB(10, 30, 10),
			ButtonColor = Color3.fromRGB(10, 30, 10),
			ButtonHoverColor = Color3.fromRGB(15, 45, 15),
			ToggleColor = Color3.fromRGB(10, 30, 10),
			ToggleColorOFF = Color3.fromRGB(50, 0, 0),
			ToggleColorON = Color3.fromRGB(0, 255, 0),
			MainFrameColor = Color3.fromRGB(0, 10, 0),
			SeparatorColor = Color3.fromRGB(0, 80, 0),
			TextBoxColor = Color3.fromRGB(5, 25, 5),
			AccentColor = Color3.fromRGB(0, 255, 0),
			SectionColor = Color3.fromRGB(0, 100, 0),
			LabelColor = Color3.fromRGB(150, 255, 150),
			SliderColor = Color3.fromRGB(0, 50, 0),
			SliderHandleColor = Color3.fromRGB(0, 255, 0),
			UIStrokeColor = Color3.fromRGB(0, 80, 0),
			LoadingBarColor = Color3.fromRGB(0, 255, 0),
			TabColor = Color3.fromRGB(10, 35, 10),
		},

		-- 5. Ocean (Deep Blue/Teal)
		Ocean = {
			TitleColor = Color3.fromRGB(200, 255, 255),
			CollapseBtnColor = Color3.fromRGB(10, 25, 35),
			SettingBtnColor = Color3.fromRGB(0, 255, 220),
			SearchBarColor = Color3.fromRGB(15, 35, 50),
			ButtonColor = Color3.fromRGB(20, 45, 60),
			ButtonHoverColor = Color3.fromRGB(30, 65, 85),
			ToggleColor = Color3.fromRGB(20, 45, 60),
			ToggleColorOFF = Color3.fromRGB(40, 80, 90),
			ToggleColorON = Color3.fromRGB(0, 200, 200),
			MainFrameColor = Color3.fromRGB(5, 15, 25),
			SeparatorColor = Color3.fromRGB(40, 90, 110),
			TextBoxColor = Color3.fromRGB(15, 35, 50),
			AccentColor = Color3.fromRGB(0, 255, 220),
			SectionColor = Color3.fromRGB(60, 120, 130),
			LabelColor = Color3.fromRGB(150, 200, 200),
			SliderColor = Color3.fromRGB(30, 70, 90),
			SliderHandleColor = Color3.fromRGB(0, 255, 220),
			UIStrokeColor = Color3.fromRGB(20, 100, 120),
			LoadingBarColor = Color3.fromRGB(0, 255, 220),
			TabColor = Color3.fromRGB(25, 55, 75),
		},

		-- 6. Royal Gold (Luxury)
		RoyalGold = {
			TitleColor = Color3.fromRGB(255, 245, 200),
			CollapseBtnColor = Color3.fromRGB(20, 20, 20),
			SettingBtnColor = Color3.fromRGB(212, 175, 55),
			SearchBarColor = Color3.fromRGB(25, 25, 25),
			ButtonColor = Color3.fromRGB(35, 35, 35),
			ButtonHoverColor = Color3.fromRGB(50, 50, 50),
			ToggleColor = Color3.fromRGB(35, 35, 35),
			ToggleColorOFF = Color3.fromRGB(120, 100, 60),
			ToggleColorON = Color3.fromRGB(255, 215, 0),
			MainFrameColor = Color3.fromRGB(15, 15, 15),
			SeparatorColor = Color3.fromRGB(80, 70, 40),
			TextBoxColor = Color3.fromRGB(25, 25, 25),
			AccentColor = Color3.fromRGB(212, 175, 55),
			SectionColor = Color3.fromRGB(100, 90, 60),
			LabelColor = Color3.fromRGB(200, 190, 150),
			SliderColor = Color3.fromRGB(60, 55, 40),
			SliderHandleColor = Color3.fromRGB(255, 215, 0),
			UIStrokeColor = Color3.fromRGB(120, 100, 50),
			LoadingBarColor = Color3.fromRGB(212, 175, 55),
			TabColor = Color3.fromRGB(40, 40, 40),
		},

		-- 7. Synthwave (Retrowave Purple/Pink)
		Synthwave = {
			TitleColor = Color3.fromRGB(255, 255, 255),
			CollapseBtnColor = Color3.fromRGB(25, 15, 45),
			SettingBtnColor = Color3.fromRGB(255, 0, 255),
			SearchBarColor = Color3.fromRGB(35, 15, 60),
			ButtonColor = Color3.fromRGB(45, 25, 75),
			ButtonHoverColor = Color3.fromRGB(65, 35, 105),
			ToggleColor = Color3.fromRGB(45, 25, 75),
			ToggleColorOFF = Color3.fromRGB(100, 40, 120),
			ToggleColorON = Color3.fromRGB(255, 0, 255),
			MainFrameColor = Color3.fromRGB(15, 5, 30),
			SeparatorColor = Color3.fromRGB(120, 40, 180),
			TextBoxColor = Color3.fromRGB(35, 15, 60),
			AccentColor = Color3.fromRGB(0, 255, 255),
			SectionColor = Color3.fromRGB(150, 50, 200),
			LabelColor = Color3.fromRGB(220, 180, 255),
			SliderColor = Color3.fromRGB(80, 40, 120),
			SliderHandleColor = Color3.fromRGB(255, 0, 255),
			UIStrokeColor = Color3.fromRGB(200, 0, 255),
			LoadingBarColor = Color3.fromRGB(255, 0, 255),
			TabColor = Color3.fromRGB(50, 20, 90),
		},

		-- 8. Sakura (Pink/White)
		Sakura = {
			TitleColor = Color3.fromRGB(255, 240, 245),
			CollapseBtnColor = Color3.fromRGB(45, 35, 40),
			SettingBtnColor = Color3.fromRGB(255, 182, 193),
			SearchBarColor = Color3.fromRGB(55, 40, 50),
			ButtonColor = Color3.fromRGB(65, 50, 60),
			ButtonHoverColor = Color3.fromRGB(85, 70, 80),
			ToggleColor = Color3.fromRGB(65, 50, 60),
			ToggleColorOFF = Color3.fromRGB(200, 100, 100),
			ToggleColorON = Color3.fromRGB(255, 150, 190),
			MainFrameColor = Color3.fromRGB(35, 25, 30),
			SeparatorColor = Color3.fromRGB(100, 70, 90),
			TextBoxColor = Color3.fromRGB(55, 40, 50),
			AccentColor = Color3.fromRGB(255, 182, 193),
			SectionColor = Color3.fromRGB(120, 90, 110),
			LabelColor = Color3.fromRGB(220, 200, 210),
			SliderColor = Color3.fromRGB(90, 70, 85),
			SliderHandleColor = Color3.fromRGB(255, 182, 193),
			UIStrokeColor = Color3.fromRGB(150, 100, 120),
			LoadingBarColor = Color3.fromRGB(255, 150, 190),
			TabColor = Color3.fromRGB(75, 55, 70),
		},

		-- 9. Magma (Orange/Charcoal)
		Magma = {
			TitleColor = Color3.fromRGB(255, 220, 200),
			CollapseBtnColor = Color3.fromRGB(20, 15, 15),
			SettingBtnColor = Color3.fromRGB(255, 65, 0),
			SearchBarColor = Color3.fromRGB(30, 20, 20),
			ButtonColor = Color3.fromRGB(40, 30, 30),
			ButtonHoverColor = Color3.fromRGB(55, 40, 40),
			ToggleColor = Color3.fromRGB(40, 30, 30),
			ToggleColorOFF = Color3.fromRGB(100, 40, 20),
			ToggleColorON = Color3.fromRGB(255, 100, 0),
			MainFrameColor = Color3.fromRGB(15, 10, 10),
			SeparatorColor = Color3.fromRGB(80, 40, 20),
			TextBoxColor = Color3.fromRGB(30, 20, 20),
			AccentColor = Color3.fromRGB(255, 65, 0),
			SectionColor = Color3.fromRGB(120, 50, 30),
			LabelColor = Color3.fromRGB(200, 160, 140),
			SliderColor = Color3.fromRGB(60, 35, 25),
			SliderHandleColor = Color3.fromRGB(255, 150, 0),
			UIStrokeColor = Color3.fromRGB(100, 40, 20),
			LoadingBarColor = Color3.fromRGB(255, 65, 0),
			TabColor = Color3.fromRGB(50, 35, 35),
		},

		-- 10. Nordic (Cool Grey/Blue)
		Nordic = {
			TitleColor = Color3.fromRGB(236, 239, 244),
			CollapseBtnColor = Color3.fromRGB(46, 52, 64),
			SettingBtnColor = Color3.fromRGB(136, 192, 208),
			SearchBarColor = Color3.fromRGB(40, 45, 55),
			ButtonColor = Color3.fromRGB(59, 66, 82),
			ButtonHoverColor = Color3.fromRGB(67, 76, 94),
			ToggleColor = Color3.fromRGB(59, 66, 82),
			ToggleColorOFF = Color3.fromRGB(191, 97, 106),
			ToggleColorON = Color3.fromRGB(163, 190, 140),
			MainFrameColor = Color3.fromRGB(35, 39, 46),
			SeparatorColor = Color3.fromRGB(76, 86, 106),
			TextBoxColor = Color3.fromRGB(46, 52, 64),
			AccentColor = Color3.fromRGB(136, 192, 208),
			SectionColor = Color3.fromRGB(76, 86, 106),
			LabelColor = Color3.fromRGB(216, 222, 233),
			SliderColor = Color3.fromRGB(59, 66, 82),
			SliderHandleColor = Color3.fromRGB(129, 161, 193),
			UIStrokeColor = Color3.fromRGB(76, 86, 106),
			LoadingBarColor = Color3.fromRGB(136, 192, 208),
			TabColor = Color3.fromRGB(59, 66, 82),
		},

		-- 11. Obsidian (Monochrome)
		Obsidian = {
			TitleColor = Color3.fromRGB(240, 240, 240),
			CollapseBtnColor = Color3.fromRGB(10, 10, 10),
			SettingBtnColor = Color3.fromRGB(255, 255, 255),
			SearchBarColor = Color3.fromRGB(20, 20, 20),
			ButtonColor = Color3.fromRGB(25, 25, 25),
			ButtonHoverColor = Color3.fromRGB(40, 40, 40),
			ToggleColor = Color3.fromRGB(25, 25, 25),
			ToggleColorOFF = Color3.fromRGB(60, 60, 60),
			ToggleColorON = Color3.fromRGB(200, 200, 200),
			MainFrameColor = Color3.fromRGB(5, 5, 5),
			SeparatorColor = Color3.fromRGB(50, 50, 50),
			TextBoxColor = Color3.fromRGB(20, 20, 20),
			AccentColor = Color3.fromRGB(255, 255, 255),
			SectionColor = Color3.fromRGB(60, 60, 60),
			LabelColor = Color3.fromRGB(180, 180, 180),
			SliderColor = Color3.fromRGB(40, 40, 40),
			SliderHandleColor = Color3.fromRGB(255, 255, 255),
			UIStrokeColor = Color3.fromRGB(80, 80, 80),
			LoadingBarColor = Color3.fromRGB(255, 255, 255),
			TabColor = Color3.fromRGB(30, 30, 30),
		},

		-- 12. Forest (Green/Brown Nature)
		Forest = {
			TitleColor = Color3.fromRGB(220, 240, 220),
			CollapseBtnColor = Color3.fromRGB(20, 25, 20),
			SettingBtnColor = Color3.fromRGB(100, 220, 100),
			SearchBarColor = Color3.fromRGB(30, 40, 30),
			ButtonColor = Color3.fromRGB(35, 45, 35),
			ButtonHoverColor = Color3.fromRGB(50, 65, 50),
			ToggleColor = Color3.fromRGB(35, 45, 35),
			ToggleColorOFF = Color3.fromRGB(180, 70, 70),
			ToggleColorON = Color3.fromRGB(100, 220, 100),
			MainFrameColor = Color3.fromRGB(15, 20, 15),
			SeparatorColor = Color3.fromRGB(60, 80, 60),
			TextBoxColor = Color3.fromRGB(30, 40, 30),
			AccentColor = Color3.fromRGB(40, 200, 120),
			SectionColor = Color3.fromRGB(80, 100, 80),
			LabelColor = Color3.fromRGB(170, 190, 170),
			SliderColor = Color3.fromRGB(50, 70, 50),
			SliderHandleColor = Color3.fromRGB(100, 220, 100),
			UIStrokeColor = Color3.fromRGB(50, 100, 50),
			LoadingBarColor = Color3.fromRGB(40, 200, 120),
			TabColor = Color3.fromRGB(40, 55, 40),
		},

		-- 13. Amethyst (Deep Purple)
		Amethyst = {
			TitleColor = Color3.fromRGB(240, 230, 255),
			CollapseBtnColor = Color3.fromRGB(25, 20, 35),
			SettingBtnColor = Color3.fromRGB(170, 120, 255),
			SearchBarColor = Color3.fromRGB(35, 30, 50),
			ButtonColor = Color3.fromRGB(40, 35, 55),
			ButtonHoverColor = Color3.fromRGB(60, 50, 80),
			ToggleColor = Color3.fromRGB(40, 35, 55),
			ToggleColorOFF = Color3.fromRGB(120, 40, 40),
			ToggleColorON = Color3.fromRGB(140, 80, 255),
			MainFrameColor = Color3.fromRGB(20, 15, 25),
			SeparatorColor = Color3.fromRGB(60, 50, 80),
			TextBoxColor = Color3.fromRGB(35, 30, 50),
			AccentColor = Color3.fromRGB(170, 120, 255),
			SectionColor = Color3.fromRGB(90, 80, 110),
			LabelColor = Color3.fromRGB(180, 170, 200),
			SliderColor = Color3.fromRGB(50, 40, 70),
			SliderHandleColor = Color3.fromRGB(170, 120, 255),
			UIStrokeColor = Color3.fromRGB(80, 60, 120),
			LoadingBarColor = Color3.fromRGB(170, 120, 255),
			TabColor = Color3.fromRGB(45, 35, 65),
		},

		-- 14. Espresso (Coffee)
		Espresso = {
			TitleColor = Color3.fromRGB(245, 235, 220),
			CollapseBtnColor = Color3.fromRGB(35, 25, 20),
			SettingBtnColor = Color3.fromRGB(230, 200, 160),
			SearchBarColor = Color3.fromRGB(45, 35, 25),
			ButtonColor = Color3.fromRGB(55, 45, 35),
			ButtonHoverColor = Color3.fromRGB(75, 65, 50),
			ToggleColor = Color3.fromRGB(55, 45, 35),
			ToggleColorOFF = Color3.fromRGB(110, 80, 70),
			ToggleColorON = Color3.fromRGB(210, 180, 140),
			MainFrameColor = Color3.fromRGB(25, 18, 15),
			SeparatorColor = Color3.fromRGB(90, 70, 50),
			TextBoxColor = Color3.fromRGB(45, 35, 25),
			AccentColor = Color3.fromRGB(230, 200, 160),
			SectionColor = Color3.fromRGB(110, 90, 70),
			LabelColor = Color3.fromRGB(210, 190, 170),
			SliderColor = Color3.fromRGB(70, 50, 40),
			SliderHandleColor = Color3.fromRGB(230, 200, 160),
			UIStrokeColor = Color3.fromRGB(100, 80, 60),
			LoadingBarColor = Color3.fromRGB(210, 180, 140),
			TabColor = Color3.fromRGB(50, 40, 30),
		},

		-- 15. Solarized (Coding)
		Solarized = {
			TitleColor = Color3.fromRGB(147, 161, 161),
			CollapseBtnColor = Color3.fromRGB(0, 33, 43),
			SettingBtnColor = Color3.fromRGB(38, 139, 210),
			SearchBarColor = Color3.fromRGB(0, 33, 43),
			ButtonColor = Color3.fromRGB(7, 54, 66),
			ButtonHoverColor = Color3.fromRGB(10, 75, 90),
			ToggleColor = Color3.fromRGB(7, 54, 66),
			ToggleColorOFF = Color3.fromRGB(220, 50, 47),
			ToggleColorON = Color3.fromRGB(133, 153, 0),
			MainFrameColor = Color3.fromRGB(0, 43, 54),
			SeparatorColor = Color3.fromRGB(88, 110, 117),
			TextBoxColor = Color3.fromRGB(0, 33, 43),
			AccentColor = Color3.fromRGB(38, 139, 210),
			SectionColor = Color3.fromRGB(101, 123, 131),
			LabelColor = Color3.fromRGB(131, 148, 150),
			SliderColor = Color3.fromRGB(7, 54, 66),
			SliderHandleColor = Color3.fromRGB(38, 139, 210),
			UIStrokeColor = Color3.fromRGB(10, 60, 75),
			LoadingBarColor = Color3.fromRGB(42, 161, 152),
			TabColor = Color3.fromRGB(0, 55, 70),
		},

		-- 16. Mint Chip (Fresh Green)
		MintChip = {
			TitleColor = Color3.fromRGB(240, 255, 245),
			CollapseBtnColor = Color3.fromRGB(20, 35, 30),
			SettingBtnColor = Color3.fromRGB(60, 255, 170),
			SearchBarColor = Color3.fromRGB(25, 45, 40),
			ButtonColor = Color3.fromRGB(35, 55, 50),
			ButtonHoverColor = Color3.fromRGB(50, 75, 70),
			ToggleColor = Color3.fromRGB(35, 55, 50),
			ToggleColorOFF = Color3.fromRGB(150, 80, 80),
			ToggleColorON = Color3.fromRGB(80, 220, 150),
			MainFrameColor = Color3.fromRGB(15, 25, 20),
			SeparatorColor = Color3.fromRGB(50, 90, 80),
			TextBoxColor = Color3.fromRGB(25, 45, 40),
			AccentColor = Color3.fromRGB(60, 255, 170),
			SectionColor = Color3.fromRGB(70, 110, 100),
			LabelColor = Color3.fromRGB(200, 230, 220),
			SliderColor = Color3.fromRGB(45, 65, 60),
			SliderHandleColor = Color3.fromRGB(60, 255, 170),
			UIStrokeColor = Color3.fromRGB(40, 100, 80),
			LoadingBarColor = Color3.fromRGB(60, 255, 170),
			TabColor = Color3.fromRGB(30, 50, 45),
		},

		-- 17. Cyber (High Tech Cyan)
		Cyber = {
			TitleColor = Color3.fromRGB(0, 240, 255),
			CollapseBtnColor = Color3.fromRGB(5, 15, 20),
			SettingBtnColor = Color3.fromRGB(0, 240, 255),
			SearchBarColor = Color3.fromRGB(8, 20, 25),
			ButtonColor = Color3.fromRGB(10, 25, 30),
			ButtonHoverColor = Color3.fromRGB(15, 40, 50),
			ToggleColor = Color3.fromRGB(10, 25, 30),
			ToggleColorOFF = Color3.fromRGB(150, 30, 30),
			ToggleColorON = Color3.fromRGB(0, 220, 255),
			MainFrameColor = Color3.fromRGB(2, 10, 12),
			SeparatorColor = Color3.fromRGB(0, 100, 120),
			TextBoxColor = Color3.fromRGB(8, 20, 25),
			AccentColor = Color3.fromRGB(0, 200, 255),
			SectionColor = Color3.fromRGB(0, 120, 150),
			LabelColor = Color3.fromRGB(180, 230, 255),
			SliderColor = Color3.fromRGB(15, 35, 45),
			SliderHandleColor = Color3.fromRGB(0, 200, 255),
			UIStrokeColor = Color3.fromRGB(0, 150, 200),
			LoadingBarColor = Color3.fromRGB(0, 200, 255),
			TabColor = Color3.fromRGB(10, 30, 40),
		},

		-- 18. Lavender (Soft Purple/White)
		Lavender = {
			TitleColor = Color3.fromRGB(80, 60, 100),
			CollapseBtnColor = Color3.fromRGB(240, 235, 250),
			SettingBtnColor = Color3.fromRGB(140, 110, 170),
			SearchBarColor = Color3.fromRGB(245, 240, 255),
			ButtonColor = Color3.fromRGB(230, 220, 240),
			ButtonHoverColor = Color3.fromRGB(210, 200, 230),
			ToggleColor = Color3.fromRGB(230, 220, 240),
			ToggleColorOFF = Color3.fromRGB(180, 120, 120),
			ToggleColorON = Color3.fromRGB(140, 110, 170),
			MainFrameColor = Color3.fromRGB(250, 245, 255),
			SeparatorColor = Color3.fromRGB(200, 190, 210),
			TextBoxColor = Color3.fromRGB(245, 240, 255),
			AccentColor = Color3.fromRGB(160, 130, 190),
			SectionColor = Color3.fromRGB(180, 170, 190),
			LabelColor = Color3.fromRGB(100, 80, 120),
			SliderColor = Color3.fromRGB(230, 220, 240),
			SliderHandleColor = Color3.fromRGB(160, 130, 190),
			UIStrokeColor = Color3.fromRGB(210, 200, 220),
			LoadingBarColor = Color3.fromRGB(160, 130, 190),
			TabColor = Color3.fromRGB(240, 230, 250),
		},

		-- 19. Midnight (Deep Navy/Silver)
		Midnight = {
			TitleColor = Color3.fromRGB(200, 210, 230),
			CollapseBtnColor = Color3.fromRGB(10, 15, 30),
			SettingBtnColor = Color3.fromRGB(255, 255, 255),
			SearchBarColor = Color3.fromRGB(15, 20, 40),
			ButtonColor = Color3.fromRGB(25, 35, 60),
			ButtonHoverColor = Color3.fromRGB(35, 45, 80),
			ToggleColor = Color3.fromRGB(25, 35, 60),
			ToggleColorOFF = Color3.fromRGB(60, 40, 40),
			ToggleColorON = Color3.fromRGB(100, 150, 255),
			MainFrameColor = Color3.fromRGB(5, 8, 20),
			SeparatorColor = Color3.fromRGB(40, 50, 90),
			TextBoxColor = Color3.fromRGB(15, 20, 40),
			AccentColor = Color3.fromRGB(100, 180, 255),
			SectionColor = Color3.fromRGB(60, 80, 130),
			LabelColor = Color3.fromRGB(170, 180, 210),
			SliderColor = Color3.fromRGB(30, 40, 70),
			SliderHandleColor = Color3.fromRGB(100, 180, 255),
			UIStrokeColor = Color3.fromRGB(50, 70, 110),
			LoadingBarColor = Color3.fromRGB(100, 180, 255),
			TabColor = Color3.fromRGB(20, 30, 55),
		},

		-- 20. Volcano (Ash/Lava)
		Volcano = {
			TitleColor = Color3.fromRGB(255, 100, 0),
			CollapseBtnColor = Color3.fromRGB(15, 10, 10),
			SettingBtnColor = Color3.fromRGB(255, 255, 0),
			SearchBarColor = Color3.fromRGB(25, 15, 15),
			ButtonColor = Color3.fromRGB(40, 20, 15),
			ButtonHoverColor = Color3.fromRGB(60, 30, 25),
			ToggleColor = Color3.fromRGB(40, 20, 15),
			ToggleColorOFF = Color3.fromRGB(80, 30, 30),
			ToggleColorON = Color3.fromRGB(255, 80, 0),
			MainFrameColor = Color3.fromRGB(10, 8, 8),
			SeparatorColor = Color3.fromRGB(100, 40, 20),
			TextBoxColor = Color3.fromRGB(25, 15, 15),
			AccentColor = Color3.fromRGB(255, 140, 0),
			SectionColor = Color3.fromRGB(120, 50, 30),
			LabelColor = Color3.fromRGB(200, 150, 130),
			SliderColor = Color3.fromRGB(50, 30, 25),
			SliderHandleColor = Color3.fromRGB(255, 200, 0),
			UIStrokeColor = Color3.fromRGB(150, 60, 30),
			LoadingBarColor = Color3.fromRGB(255, 100, 0),
			TabColor = Color3.fromRGB(35, 20, 20),
		},

		-- 21. Toxic (Acid Green/Black)
		Toxic = {
			TitleColor = Color3.fromRGB(200, 255, 0),
			CollapseBtnColor = Color3.fromRGB(10, 15, 5),
			SettingBtnColor = Color3.fromRGB(255, 255, 255),
			SearchBarColor = Color3.fromRGB(20, 30, 10),
			ButtonColor = Color3.fromRGB(30, 45, 15),
			ButtonHoverColor = Color3.fromRGB(45, 60, 25),
			ToggleColor = Color3.fromRGB(30, 45, 15),
			ToggleColorOFF = Color3.fromRGB(60, 0, 0),
			ToggleColorON = Color3.fromRGB(200, 255, 0),
			MainFrameColor = Color3.fromRGB(5, 8, 2),
			SeparatorColor = Color3.fromRGB(80, 120, 30),
			TextBoxColor = Color3.fromRGB(20, 30, 10),
			AccentColor = Color3.fromRGB(180, 255, 50),
			SectionColor = Color3.fromRGB(100, 150, 40),
			LabelColor = Color3.fromRGB(200, 230, 150),
			SliderColor = Color3.fromRGB(40, 60, 20),
			SliderHandleColor = Color3.fromRGB(180, 255, 50),
			UIStrokeColor = Color3.fromRGB(100, 180, 50),
			LoadingBarColor = Color3.fromRGB(180, 255, 50),
			TabColor = Color3.fromRGB(25, 40, 15),
		},

		-- 22. Ghost (Transparent/White/Grey)
		Ghost = {
			TitleColor = Color3.fromRGB(255, 255, 255),
			CollapseBtnColor = Color3.fromRGB(20, 20, 20),
			SettingBtnColor = Color3.fromRGB(200, 200, 200),
			SearchBarColor = Color3.fromRGB(35, 35, 35),
			ButtonColor = Color3.fromRGB(45, 45, 45),
			ButtonHoverColor = Color3.fromRGB(60, 60, 60),
			ToggleColor = Color3.fromRGB(45, 45, 45),
			ToggleColorOFF = Color3.fromRGB(100, 100, 100),
			ToggleColorON = Color3.fromRGB(255, 255, 255),
			MainFrameColor = Color3.fromRGB(25, 25, 25),
			SeparatorColor = Color3.fromRGB(100, 100, 100),
			TextBoxColor = Color3.fromRGB(35, 35, 35),
			AccentColor = Color3.fromRGB(200, 200, 200),
			SectionColor = Color3.fromRGB(80, 80, 80),
			LabelColor = Color3.fromRGB(220, 220, 220),
			SliderColor = Color3.fromRGB(60, 60, 60),
			SliderHandleColor = Color3.fromRGB(255, 255, 255),
			UIStrokeColor = Color3.fromRGB(120, 120, 120),
			LoadingBarColor = Color3.fromRGB(255, 255, 255),
			TabColor = Color3.fromRGB(40, 40, 40),
		},
	}

UILibrary.DefaultConfig = {
	Title = "UI Library",
	TitleText = "UI Library",
	Size = UDim2.new(0, 250, 0, 350),
	Position = UDim2.new(0.5, -125, 0.5, -175),
	SettingSize = UDim2.new(0, 250, 0, 100),
	TitleHeight = 30,
	CornerRadius = 6,
	ElementPadding = 6,
	Font = Enum.Font.GothamBold,
	TextSize = 12,
	SectionHeight = 20,
	UIStrokeThickness = 1,
	UIStrokeEnabled = true,
	NotifcationSound = "rbxassetid://80833448337193",
	ResetOnSpawn = false,
	Addons = {},
	insertKeyAtEnd = true,
	Key = "Testing1",
	KeyLink = "SomethingNotNil",
	KeySystemEnabled = false,
	TabPadding = 10,
	UseOwnTheme = false,
	Theme = "Magma",
	SaveOnExit = true,
	KeyCallBack = function()

	end,
}

local TS = game:GetService("TweenService")
local HS = game:GetService("HttpService")
local UIS = game:GetService("UserInputService")
local RS = game:GetService("RunService")
local Players = game:GetService("Players")
local CoreGui = game:GetService("CoreGui")

local LocalPlayer = Players.LocalPlayer
local Mouse = LocalPlayer:GetMouse()

UILibrary.Utility = {
	-- [ FILE SYSTEM & DATA ] --

	WriteFile = function(self, config)
		local path = config.Path
		local data = config.Data or ""
		if config.IsJSON and type(data) == "table" then
			data = HS:JSONEncode(data)
		else
			data = tostring(data)
		end
		writefile(path, data)
	end,

	ReadFile = function(self, config)
		local path = config.Path
		if not isfile(path) then return nil end
		local data = readfile(path)
		if config.IsJSON then
			local success, decoded = pcall(function() return HS:JSONDecode(data) end)
			return success and decoded or nil
		end
		return data
	end,

	CreateFolder = function(self, config)
		local name = config.Name
		if not isfolder(name) then
			makefolder(name)
		end
		return name
	end,

	MergeTables = function(self, t1, t2)
		local newTable = t1
		for k, v in pairs(t2) do
			if type(v) == "table" and type(newTable[k]) == "table" then
				newTable[k] = self:MergeTables(newTable[k], v)
			else
				newTable[k] = v
			end
		end
		return newTable
	end,

	SafeLoadstring = function(self, url)
		local s, code = pcall(function() return game:HttpGet(url) end)
		if not s then warn("URL FETCH FAILED: " .. url) return nil end
		local func, errorMessage = loadstring(code)
		if func then return func() else
			warn("LOADSTRING FAILED: " .. tostring(errorMessage))
			return nil
		end
	end,

	-- [ UI VISUALS & ANIMATION ] --

	Tween = function(self, config)
		local obj = config.Object
		local goal = config.Goal
		local info = TweenInfo.new(
			config.Time or 0.5, 
			config.Style or Enum.EasingStyle.Quart, 
			config.Direction or Enum.EasingDirection.Out
		)
		local tween = TS:Create(obj, info, goal)
		tween:Play()
		if config.Wait then tween.Completed:Wait() end
		return tween
	end,

	Ripple = function(self, config)
		local btn = config.Parent
		local x, y = config.X or Mouse.X, config.Y or Mouse.Y
		task.spawn(function()
			local circle = Instance.new("ImageLabel")
			circle.Name = "Ripple"
			circle.BackgroundTransparency = 0.6
			circle.Image = "rbxassetid://266543268"
			circle.ImageTransparency = 0.8
			circle.ZIndex = 10
			circle.Parent = btn

			local btnSize = btn.AbsoluteSize
			local maxSize = math.max(btnSize.X, btnSize.Y) * 1.5
			local relativeX, relativeY = x - btn.AbsolutePosition.X, y - btn.AbsolutePosition.Y

			circle.Position = UDim2.new(0, relativeX, 0, relativeY)
			circle.Size = UDim2.new(0, 0, 0, 0)

			local tween = TS:Create(circle, TweenInfo.new(0.4, Enum.EasingStyle.Quart), {
				Size = UDim2.new(0, maxSize, 0, maxSize),
				Position = UDim2.new(0, relativeX - maxSize/2, 0, relativeY - maxSize/2),
				ImageTransparency = 1,
				BackgroundTransparency = 1
			})
			tween:Play()
			tween.Completed:Connect(function() circle:Destroy() end)
		end)
	end,

	MakeDraggable = function(self, config)
		local frame = config.Frame
		local dragObj = config.DragObject or frame
		local dragging, dragInput, dragStart, startPos

		dragObj.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				dragging = true
				dragStart = input.Position
				startPos = frame.Position
				input.Changed:Connect(function()
					if input.UserInputState == Enum.UserInputState.End then dragging = false end
				end)
			end
		end)

		UIS.InputChanged:Connect(function(input)
			if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
				local delta = input.Position - dragStart
				TS:Create(frame, TweenInfo.new(0.05), {
					Position = UDim2.new(
						startPos.X.Scale, startPos.X.Offset + delta.X,
						startPos.Y.Scale, startPos.Y.Offset + delta.Y
					)
				}):Play()
			end
		end)
	end,

	-- [ NETWORKING & ASSETS ] --

	GetImage = function(self, config)
		local fileName = config.Name .. ".png"
		if not isfile(fileName) then
			local s, content = pcall(function() return game:HttpGet(config.Url) end)

			if s then 
				self:WriteFile({Path = fileName, Data = content, IsJSON = false}) 
			end
		end
		return getcustomasset(fileName)
	end,

	-- [ MISC UTILITIES ] --

	ProtectGui = function(self, gui)
		local success, coreGui = pcall(function()
			return game:GetService("CoreGui") 
		end)

		if gethui then
			gui.Parent = gethui()
		elseif syn and syn.protect_gui then
			syn.protect_gui(gui) gui.Parent = CoreGui
		elseif success and coreGui then
			gui.Parent = CoreGui
		else
			gui.Parent = LocalPlayer:WaitForChild("PlayerGui")
		end
	end,

	GetRainbow = function(self, speed)
		return Color3.fromHSV(tick() % (speed or 5) / (speed or 5), 1, 1)
	end,

	FormatKey = function(self, keyCode)
		if not keyCode then return "None" end
		local replacements = {
			["LeftShift"] = "LShift", ["RightShift"] = "RShift",
			["LeftControl"] = "LCtrl", ["RightControl"] = "RCtrl",
			["Return"] = "Enter", ["MouseButton1"] = "M1", ["MouseButton2"] = "M2"
		}
		return replacements[keyCode.Name] or keyCode.Name
	end,

	Connect = function(self, signal, callback, storageTable)
		local connection = signal:Connect(callback)
		if storageTable then table.insert(storageTable, connection) end
		return connection
	end
}

local utils = UILibrary.Utility

UILibrary.Main = {
	new = function(config)
		local self = setmetatable({}, { __index = UILibrary.Main})

		self.Config = {}
		self.UserConfig = {}
		self.UserSettings = {}

		for k, v in pairs(UILibrary.DefaultConfig) do
			self.Config[k] = (config and config[k] ~= nil) and config[k] or v
		end

		self.HubFolder = utils:CreateFolder({Name = self.Config.Title})
		self.KeyFile = self.HubFolder .. "/Key.txt"
		self.UserConfigFile = self.HubFolder.."/UserConfig"
		self.UserSettingFile = self.HubFolder.."/UserSettings"
		
		self.Config.KeyLink = config.KeyLink

		self.Colors = {}

		if self.Config.UseOwnTheme then
			for colorName, defaultColor in pairs(UILibrary.DefaultThemes.Default) do
				self.Colors[colorName] = config.Colors[colorName] or defaultColor
			end
		elseif self.Config.Theme then
			for colorName, Color in pairs(UILibrary.DefaultThemes[self.Config.Theme]) do
				self.Colors[colorName] = Color
			end
		else
			for colorName, Color in pairs(UILibrary.DefaultThemes.Default) do
				self.Colors[colorName] = Color
			end
		end

		self.Elements = {}
		self.SettingElements = {}

		self.Tabs = {}

		self.Sections = {}

		self.FVisible = true
		self.GVisible = true

		self.Minimized = false

		self:LoadConfigurations()

		self:CreateScreenGui()

		self:Intro()
		
		task.wait(1)

		local savedKey = isfile(self.KeyFile) and utils:ReadFile({Path = self.KeyFile, IsJSON = false})

		if self.Config.KeySystemEnabled then
			if savedKey then
				if self:CheckKey(savedKey) then
					if self.Config.KeyCallBack then
						self.Config.KeyCallBack() 
					end
				else
					self:CreateKeyUI()
				end
			else
				self:CreateKeyUI()
			end
		end

		if self.Config.SaveOnExit then
			LocalPlayer.AncestryChanged:Connect(function(_, parent)
				if not parent then
					self:SaveConfigurations()
				end
			end)
		end

		return self
	end,

	CreateScreenGui = function(self)
		if not game:IsLoaded() then game.Loaded:Wait() end

		self.ScreenGui = Instance.new("ScreenGui")
		self.ScreenGui.Name = "SBEUILibrary"
		self.ScreenGui.DisplayOrder = 999999999 
		self.ScreenGui.IgnoreGuiInset = true 

		utils:ProtectGui(self.ScreenGui)

		self.ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
		self.ScreenGui.ResetOnSpawn = self.Config.ResetOnSpawn
	end,

	Intro = function(self)
		self.Image = Instance.new("ImageLabel")
		self.Image.Size = UDim2.new(0, 500, 0, 500)
		self.Image.Position = UDim2.new(0.5, 0, 0.5, 0)
		self.Image.AnchorPoint = Vector2.new(0.5, 0.5)
		self.Image.Image = utils:GetImage({Name = "SBEUILib", Url = "https://i.ibb.co/xK20Pwph/SBEUILib.png"})
		self.Image.BackgroundTransparency = 1
		self.Image.ImageTransparency = 0
		self.Image.Parent = self.ScreenGui
		self.Image.ZIndex = 999

		task.wait(2)

		local fadeInfo = TweenInfo.new(1.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
		local fadeTween = TS:Create(self.Image, fadeInfo, {
			ImageTransparency = 1
		})

		fadeTween:Play()

		fadeTween.Completed:Connect(function()
			self.Image:Destroy()
		end)
	end,

	CheckKey = function(self, key)
		if key == "" or key == nil then return false end

		local target = self.Config.Key

		if not string.find(target, "http://") and not string.find(target, "https://") then
			return key == target
		end

		local url = target
		if self.Config.insertKeyAtEnd == true then
			url = url .. key
		end

		local s, res = pcall(function() 
			return game:HttpGet(url) 
		end)

		if s and res then
			local jsonSuccess, data = pcall(function() 
				return game:GetService("HttpService"):JSONDecode(res) 
			end)

			if jsonSuccess and type(data) == "table" then
				print(data.success)
				return data.success == true or data.valid == true
			else
				local cleanRes = string.gsub(res, "%s+", "")
				return key == cleanRes
			end
		end

		return false
	end,

	CreateUI = function(self)
		self.UIWrapper = Instance.new("Frame")
		self.UIWrapper.Name = "UIWrapper"
		self.UIWrapper.Size = self.Config.Size
		self.UIWrapper.Position = self.Config.Position
		self.UIWrapper.BackgroundTransparency = 1
		self.UIWrapper.Active = true
		self.UIWrapper.Draggable = true
		self.UIWrapper.Parent = self.ScreenGui

		self.MainFrame = Instance.new("Frame")
		self.MainFrame.Name = "MainFrame"
		self.MainFrame.Size = UDim2.new(1, 0, 1, 0)
		self.MainFrame.BackgroundColor3 = self.Colors.MainFrameColor
		self.MainFrame.BorderSizePixel = 0
		self.MainFrame.ClipsDescendants = true
		self.MainFrame.Parent = self.UIWrapper

		local corner = Instance.new("UICorner")
		corner.CornerRadius = UDim.new(0, self.Config.CornerRadius)
		corner.Parent = self.MainFrame

		if self.Config.UIStrokeEnabled then
			local uiStroke = Instance.new("UIStroke")
			uiStroke.Thickness = self.Config.UIStrokeThickness
			uiStroke.Color = self.Colors.UIStrokeColor
			uiStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
			uiStroke.Parent = self.MainFrame
		end

		self.TitleBar = Instance.new("Frame")
		self.TitleBar.Name = "TitleBar"
		self.TitleBar.Size = UDim2.new(1, 0, 0, self.Config.TitleHeight + 35) 
		self.TitleBar.Position = UDim2.new(0, 0, 0, 0)
		self.TitleBar.BackgroundColor3 = self.Colors.CollapseBtnColor
		self.TitleBar.BorderSizePixel = 0
		self.TitleBar.ZIndex = 2
		self.TitleBar.Parent = self.MainFrame

		local titleCorner = Instance.new("UICorner")
		titleCorner.CornerRadius = UDim.new(0, self.Config.CornerRadius)
		titleCorner.Parent = self.TitleBar

		self.TitleText = Instance.new("TextLabel")
		self.TitleText.Name = "TitleText"

		self.TitleText.Size = UDim2.new(0.6, 0, 1, 0)
		self.TitleText.Position = UDim2.new(0, 12, -0.25, 0)
		self.TitleText.BackgroundTransparency = 1
		self.TitleText.Text = self.Config.TitleText
		self.TitleText.TextColor3 = self.Colors.TitleColor
		self.TitleText.TextXAlignment = Enum.TextXAlignment.Left
		self.TitleText.Font = Enum.Font.GothamBold
		self.TitleText.TextSize = 14
		self.TitleText.ZIndex = 3
		self.TitleText.Parent = self.TitleBar

		self.SearchFrame = Instance.new("Frame")
		self.SearchFrame.Name = "SearchFrame"
		self.SearchFrame.Parent = self.TitleBar
		self.SearchFrame.Size = UDim2.new(1, -44, 0.4, 0)
		self.SearchFrame.Position = UDim2.new(0, 6, 0.5, 0)
		self.SearchFrame.BackgroundColor3 = self.Colors.SearchBarColor
		self.SearchFrame.BackgroundTransparency = 0.2
		self.SearchFrame.ZIndex = 3

		local searchCorner = Instance.new("UICorner")
		searchCorner.CornerRadius = UDim.new(0, 6)
		searchCorner.Parent = self.SearchFrame

		self.TabSearch = Instance.new("TextBox")
		self.TabSearch.Name = "TabSearch"
		self.TabSearch.Parent = self.SearchFrame
		self.TabSearch.Size = UDim2.new(1, 0, 1, 0)
		self.TabSearch.Position = UDim2.new(0, 10, 0, 0)
		self.TabSearch.BackgroundTransparency = 1
		self.TabSearch.PlaceholderText = "Search tabs..."
		self.TabSearch.Text = ""
		self.TabSearch.TextColor3 = Color3.new(1, 1, 1)
		self.TabSearch.PlaceholderColor3 = Color3.fromRGB(150, 150, 150)
		self.TabSearch.TextSize = 12
		self.TabSearch.Font = Enum.Font.Gotham
		self.TabSearch.TextXAlignment = Enum.TextXAlignment.Left
		self.TabSearch.ClearTextOnFocus = false
		self.TabSearch.ZIndex = 4

		self.ClearSearch = Instance.new("TextButton")
		self.ClearSearch.Name = "ClearSearch"
		self.ClearSearch.Parent = self.SearchFrame
		self.ClearSearch.Size = UDim2.new(0, 20, 0, 20)
		self.ClearSearch.Position = UDim2.new(1, -25, 0.5, -10)
		self.ClearSearch.BackgroundTransparency = 1
		self.ClearSearch.Text = "×"
		self.ClearSearch.TextColor3 = Color3.fromRGB(200, 200, 200)
		self.ClearSearch.TextSize = 18
		self.ClearSearch.Visible = false
		self.ClearSearch.ZIndex = 4

		self.ScrollingFrame = Instance.new("ScrollingFrame")
		self.ScrollingFrame.Name = "ContentFrame"
		self.ScrollingFrame.Size = UDim2.new(1, 0, 1, -(self.Config.TitleHeight + 35))
		self.ScrollingFrame.Position = UDim2.new(0, 0, 0, self.Config.TitleHeight + 35)
		self.ScrollingFrame.BackgroundTransparency = 1
		self.ScrollingFrame.BorderSizePixel = 0
		self.ScrollingFrame.ScrollBarThickness = 4
		self.ScrollingFrame.ScrollBarImageColor3 = self.Colors.AccentColor
		self.ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, 0)
		self.ScrollingFrame.Parent = self.MainFrame

		self.TabScrollingFrame = Instance.new("ScrollingFrame")
		self.TabScrollingFrame.Name = "TabFrame"
		self.TabScrollingFrame.Size = UDim2.new(1, -24, 1, -(self.Config.TitleHeight + 35))
		self.TabScrollingFrame.Position = UDim2.new(1, -34, 0, self.Config.TitleHeight + 35)
		self.TabScrollingFrame.BackgroundTransparency = 1
		self.TabScrollingFrame.BorderSizePixel = 0
		self.TabScrollingFrame.ScrollBarThickness = 4
		self.TabScrollingFrame.ScrollBarImageColor3 = self.Colors.AccentColor
		self.TabScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, 0)
		self.TabScrollingFrame.Parent = self.MainFrame

		local TopBtns = Instance.new("Frame", self.TitleBar)
		TopBtns.Size = UDim2.new(0.3, 0, 1, 0)
		TopBtns.Position = UDim2.new(1, 0, -0.25, 0)
		TopBtns.AnchorPoint = Vector2.new(1, 0)
		TopBtns.BackgroundTransparency = 1

		local BtnLayout = Instance.new("UIListLayout", TopBtns)
		BtnLayout.FillDirection = Enum.FillDirection.Horizontal
		BtnLayout.HorizontalAlignment = Enum.HorizontalAlignment.Right
		BtnLayout.VerticalAlignment = Enum.VerticalAlignment.Center
		BtnLayout.Padding = UDim.new(0, 5)

		self.MinimizeButton = Instance.new("TextButton", TopBtns)
		self.MinimizeButton.Size = UDim2.new(0, 25, 0, 25)
		self.MinimizeButton.BackgroundTransparency = 1
		self.MinimizeButton.Text = "-"
		self.MinimizeButton.TextColor3 = self.Colors.TitleColor
		self.MinimizeButton.TextSize = 20
		self.MinimizeButton.Font = Enum.Font.GothamBold

		self.CloseBtn = Instance.new("TextButton", TopBtns)
		self.CloseBtn.Size = UDim2.new(0, 25, 0, 25)
		self.CloseBtn.BackgroundTransparency = 1
		self.CloseBtn.Text = "×"
		self.CloseBtn.TextColor3 = Color3.fromRGB(255, 100, 100)
		self.CloseBtn.TextSize = 20
		self.CloseBtn.Font = Enum.Font.GothamBold

		self.UIListLayout = Instance.new("UIListLayout")
		self.UIListLayout.Padding = UDim.new(0, self.Config.ElementPadding)
		self.UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
		self.UIListLayout.Parent = self.ScrollingFrame

		self.UIListLayout2 = self.UIListLayout:Clone()
		self.UIListLayout2.Padding = UDim.new(0, self.Config.TabPadding)
		self.UIListLayout2.Parent = self.TabScrollingFrame
			
		self.UIListLayout3 = self.UIListLayout:Clone()
		self.UIListLayout3.Padding = UDim.new(0, self.Config.ElementPadding)
		self.UIListLayout3.Parent = self.SettingScrollingFrame
		
		local padding = Instance.new("UIPadding")
		padding.PaddingLeft = UDim.new(0, 12)
		padding.PaddingRight = UDim.new(0, 12)
		padding.PaddingTop = UDim.new(0, 10)
		padding.PaddingBottom = UDim.new(0, 10)
		padding.Parent = self.ScrollingFrame

		local padding2 = padding:Clone()
		padding2.Parent = self.TabScrollingFrame
		
		local padding3 = padding:Clone()
		padding3.Parent = self.SettingScrollingFrame
		
		self.SettingsBtn = Instance.new("TextButton")
		self.SettingsBtn.Name = "SettingsBtn"
		self.SettingsBtn.Parent = self.TitleBar
		self.SettingsBtn.Position = UDim2.new(1, -32.5, 0.5, 0) 
		self.SettingsBtn.Size = UDim2.new(0, 25, 0, 25)
		self.SettingsBtn.BackgroundTransparency = 1
		self.SettingsBtn.Text = "⚙"
		self.SettingsBtn.TextColor3 = self.Colors.SettingBtnColor
		self.SettingsBtn.TextSize = 18
		self.SettingsBtn.Font = Enum.Font.Gotham
		self.SettingsBtn.ZIndex = 4

		self.SettingsVisible = false

		self.SettingsFrame = Instance.new("Frame")
		self.SettingsFrame.Name = "SettingsFrame"
		self.SettingsFrame.Size = UDim2.new(0, 0, 1, 0)
		self.SettingsFrame.Position = UDim2.new(1, 10, 0, 0)
		self.SettingsFrame.BackgroundColor3 = self.Colors.MainFrameColor
		self.SettingsFrame.BorderSizePixel = 0
		self.SettingsFrame.ClipsDescendants = true
		self.SettingsFrame.Visible = false
		self.SettingsFrame.Parent = self.UIWrapper

		local settingsCorner = Instance.new("UICorner")
		settingsCorner.CornerRadius = UDim.new(0, self.Config.CornerRadius)
		settingsCorner.Parent = self.SettingsFrame

		local settingsTitle = Instance.new("TextLabel", self.SettingsFrame)
		settingsTitle.Size = UDim2.new(1, 0, 0, 40)
		settingsTitle.BackgroundTransparency = 1
		settingsTitle.Text = "SETTINGS"
		settingsTitle.TextColor3 = self.Colors.TitleColor
		settingsTitle.Font = Enum.Font.GothamBold
		settingsTitle.TextSize = 14

		self.SettingsBtn.MouseButton1Click:Connect(function()
			self.SettingsVisible = not self.SettingsVisible
			local targetSize = self.SettingsVisible and self.Config.SettingSize or UDim2.new(0, 0, 1, 0)

			if self.SettingsVisible then
				self.SettingsFrame.Visible = true
			end

			local tween = TS:Create(self.SettingsFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Size = targetSize
			})

			tween:Play()

			tween.Completed:Connect(function()
				if not self.SettingsVisible then
					self.SettingsFrame.Visible = false
				end
			end)
		end)

		self.SettingScrollingFrame = Instance.new("ScrollingFrame")
		self.SettingScrollingFrame.Name = "ContentFrame"
		self.SettingScrollingFrame.Size = UDim2.new(1, 0, 1, -40)
		self.SettingScrollingFrame.Position = UDim2.new(0, 0, 0, 40)
		self.SettingScrollingFrame.BackgroundTransparency = 1
		self.SettingScrollingFrame.BorderSizePixel = 0
		self.SettingScrollingFrame.ScrollBarThickness = 4
		self.SettingScrollingFrame.ScrollBarImageColor3 = self.Colors.AccentColor
		self.SettingScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, 0)
		self.SettingScrollingFrame.Parent = self.SettingsFrame

		self:AddSettingKeybind({
		    Text = "Toggle SBEUI GUI",
		    Default = Enum.KeyCode.F2,
		    Callback = function()
		        self:ToggleGuiVisibility()
		    end
		})

		self:AddSettingKeybind({
		    Text = "Toggle SBEUI Frame",
		    Default = Enum.KeyCode.F3,
		    Callback = function()
		        self:ToggleFrameVisibility()
		    end
		})
	

		self.OriginalSize = self.MainFrame.Size
		self.OriginalPosition = self.MainFrame.Position

		self.TabSearch:GetPropertyChangedSignal("Text"):Connect(function()
			local query = string.lower(self.TabSearch.Text)
			self.ClearSearch.Visible = (query ~= "")

			for _, tab in pairs(self.Tabs or {}) do
				local btn = tab:FindFirstChildOfClass("TextButton") or tab:FindFirstChildOfClass("TextLabel")
				local tabName = btn and btn.Text or tab.Name

				if string.find(string.lower(tabName), query) then
					tab.Visible = true
				else
					tab.Visible = false
				end
			end
		end)

		self.ClearSearch.MouseButton1Click:Connect(function()
			self.TabSearch.Text = ""
		end)

		self.CloseBtn.MouseButton1Click:Connect(function()
			self:PromptDestroy()
		end)

		self.MinimizeButton.MouseButton1Click:Connect(function()
			self:ToggleMinimize()
		end)

		self.UIListLayout2:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
			self.TabScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, self.UIListLayout2.AbsoluteContentSize.Y + 15)
		end)

		self.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
			self.ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, self.UIListLayout.AbsoluteContentSize.Y + 15)
		end)
		
		self.UIListLayout3:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
			self.SettingScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, self.UIListLayout3.AbsoluteContentSize.Y + 15)
		end)

		local lastActiveTab = nil

		self.ScrollingFrame:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
			local scrollY = self.ScrollingFrame.CanvasPosition.Y
			local frameTop = self.ScrollingFrame.AbsolutePosition.Y
			local activeSection = nil

			for i = #self.Sections, 1, -1 do
				local section = self.Sections[i]
				if section.AbsolutePosition.Y <= (frameTop + 50) then -- 50px buffer for better feel
					activeSection = section
					break 
				end
			end

			if activeSection then
				local targetTabName = activeSection:GetAttribute("TabName")

				if targetTabName ~= lastActiveTab then
					lastActiveTab = targetTabName

					for _, t in pairs(self.Tabs) do
						local isMatch = (t.Name == targetTabName)

						TS:Create(t.Accent, TweenInfo.new(0.2), {
							Size = isMatch and UDim2.new(0, 2, 0, 40) or UDim2.new(0, 2, 0, 0)
						}):Play()

						TS:Create(t, TweenInfo.new(0.2), {
							GroupTransparency = isMatch and 0 or 0.3,
							BackgroundColor3 = isMatch and self.Colors.AccentColor:Lerp(self.Colors.TabColor, 0.7) or self.Colors.TabColor
						}):Play()
					end
				end
			end
		end)

		self.TitleBar.MouseEnter:Connect(function()
			self.TitleBar.BackgroundColor3 = Color3.new(
				self.Colors.CollapseBtnColor.R * 1.2,
				self.Colors.CollapseBtnColor.G * 1.2,
				self.Colors.CollapseBtnColor.B * 1.2
			)
		end)

		self.TitleBar.MouseLeave:Connect(function()
			self.TitleBar.BackgroundColor3 = self.Colors.CollapseBtnColor
		end)
	end,


	CreateKeyUI = function(self) -- yo azka design this key system you monkey
		self.KeyFrame = Instance.new("Frame")
		self.KeyFrame.Name = "KeyFrame"
		self.KeyFrame.Size = UDim2.new(0, 340, 0, 180)
		self.KeyFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
		self.KeyFrame.AnchorPoint = Vector2.new(0.5, 0.5)
		self.KeyFrame.BackgroundColor3 = self.Colors.MainFrameColor
		self.KeyFrame.Parent = self.ScreenGui

		Instance.new("UICorner", self.KeyFrame).CornerRadius = UDim.new(0, 8)

		if self.Config.UIStrokeEnabled then
			local s = Instance.new("UIStroke", self.KeyFrame)
			s.Color = self.Colors.UIStrokeColor
			s.Thickness = self.Config.UIStrokeThickness
		end

		local TitleBar = Instance.new("Frame", self.KeyFrame)
		TitleBar.Size = UDim2.new(1, 0, 0, 35)
		TitleBar.BackgroundTransparency = 1

		local Title = Instance.new("TextLabel", TitleBar)
		Title.Size = UDim2.new(1, -40, 1, 0)
		Title.Position = UDim2.new(0, 12, 0, 0)
		Title.BackgroundTransparency = 1
		Title.Text = self.Config.TitleText .. " | Key System"
		Title.TextColor3 = self.Colors.TitleColor
		Title.Font = Enum.Font.GothamBold
		Title.TextSize = 14
		Title.TextXAlignment = Enum.TextXAlignment.Left

		local CloseBtn = Instance.new("TextButton", TitleBar)
		CloseBtn.Name = "CloseButton"
		CloseBtn.Size = UDim2.new(0, 35, 0, 35)
		CloseBtn.Position = UDim2.new(1, -35, 0, 0)
		CloseBtn.BackgroundTransparency = 1
		CloseBtn.Text = "×" 
		CloseBtn.TextColor3 = Color3.fromRGB(200, 50, 50)
		CloseBtn.TextSize = 22
		CloseBtn.Font = Enum.Font.GothamBold

		CloseBtn.MouseButton1Click:Connect(function()
			self:PromptDestroy()
		end)

		self.KeyBox = Instance.new("TextBox", self.KeyFrame)
		self.KeyBox.Size = UDim2.new(0.9, 0, 0, 40)
		self.KeyBox.Position = UDim2.new(0.05, 0, 0.35, 0)
		self.KeyBox.BackgroundColor3 = self.Colors.CollapseBtnColor
		self.KeyBox.PlaceholderText = "Paste Key..."
		self.KeyBox.Text = ""
		self.KeyBox.TextColor3 = Color3.new(1, 1, 1)
		self.KeyBox.Font = Enum.Font.GothamMedium
		self.KeyBox.TextSize = 16
		Instance.new("UICorner", self.KeyBox).CornerRadius = UDim.new(0, 6)

		local ButtonContainer = Instance.new("Frame", self.KeyFrame)
		ButtonContainer.Size = UDim2.new(0.9, 0, 0, 35)
		ButtonContainer.Position = UDim2.new(0.05, 0, 0.7, 0)
		ButtonContainer.BackgroundTransparency = 1

		local Layout = Instance.new("UIListLayout", ButtonContainer)
		Layout.FillDirection = Enum.FillDirection.Horizontal
		Layout.Padding = UDim.new(0, 10)
		Layout.HorizontalAlignment = Enum.HorizontalAlignment.Center

		local function verify()
			local entered = self.KeyBox.Text
			if entered == "" then return end
			local isCorrect = self:CheckKey(entered)
			if isCorrect then
				self.KeyBox.Text = "Access Granted!"
				local t = game:GetService("TweenService"):Create(self.KeyFrame, TweenInfo.new(0.3), {
					Size = UDim2.new(0,0,0,0), 
					BackgroundTransparency = 1
				})
				t:Play()
				t.Completed:Connect(function()
					self.KeyFrame:Destroy()

					utils:WriteFile({Path = self.KeyFile, Data = entered, IsJSON = false})

					if self.Config.KeyCallBack then 
						self.Config.KeyCallBack() 
					end
				end)
			else
				self.KeyBox.Text = ""
				self.KeyBox.PlaceholderText = "Invalid Key!"
				task.wait(1)
				self.KeyBox.PlaceholderText = "Paste Key..."
			end
		end

		local Submit = Instance.new("TextButton", ButtonContainer)
		Submit.Size = UDim2.new(0.48, 0, 1, 0)
		Submit.BackgroundColor3 = Color3.fromRGB(0, 160, 0)
		Submit.Text = "Verify"
		Submit.Font = Enum.Font.GothamBold
		Submit.TextColor3 = Color3.new(1, 1, 1)
		Submit.TextSize = 14
		Instance.new("UICorner", Submit).CornerRadius = UDim.new(0, 6)
		Submit.MouseButton1Click:Connect(verify)

		local GetKey = Instance.new("TextButton", ButtonContainer)
		GetKey.Size = UDim2.new(0.48, 0, 1, 0)
		GetKey.BackgroundColor3 = Color3.fromRGB(60, 60, 60)
		GetKey.Text = "Get Key"
		GetKey.Font = Enum.Font.GothamBold
		GetKey.TextColor3 = Color3.new(1, 1, 1)
		GetKey.TextSize = 14
		Instance.new("UICorner", GetKey).CornerRadius = UDim.new(0, 6)

		GetKey.MouseButton1Click:Connect(function()
			local url = self.Config.KeyLink or ""

			if self.Config.Addons and type(self.Config.Addons) == "table" then
				local isFirst = not string.find(url, "?")

				for key, value in pairs(self.Config.Addons) do
					local separator = isFirst and "?" or "&"
					url = url .. separator .. tostring(key) .. "=" .. tostring(value)
					isFirst = false
				end
			end

			setclipboard(url)

			self:Notify({
				Title = "Link Copied!",
				Text = "Link successfully copied to clipboard.",
				Duration = 3,
				StartPosition = UDim2.new(1, 0, 0.776, 0),
				EndPosition = UDim2.new(0.724, 0, 0.776, 0),
				TweenTime = 0.5
			})
		end)
	end,

	CreateFakeLoading = function(self, config)
		self.LoadFrame = Instance.new("Frame")
		self.LoadFrame.Name = "LoadFrame"
		self.LoadFrame.Size = config.Size
		self.LoadFrame.Position = config.Position
		self.LoadFrame.BackgroundColor3 = self.Colors.MainFrameColor
		self.LoadFrame.BorderSizePixel = 0
		self.LoadFrame.Active = true
		self.LoadFrame.Draggable = true
		self.LoadFrame.ClipsDescendants = true
		self.LoadFrame.AnchorPoint = Vector2.new(0.5, 0.5)
		self.LoadFrame.Parent = self.ScreenGui

		local corner = Instance.new("UICorner")
		corner.CornerRadius = UDim.new(0, self.Config.CornerRadius)
		corner.Parent = self.LoadFrame

		if self.Config.UIStrokeEnabled then
			local uiStroke = Instance.new("UIStroke")
			uiStroke.Thickness = self.Config.UIStrokeThickness
			uiStroke.Color = self.Colors.UIStrokeColor
			uiStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
			uiStroke.Parent = self.LoadFrame
		end

		self.LoadTitleBar = Instance.new("Frame")
		self.LoadTitleBar.Name = "LoadTitleBar"
		self.LoadTitleBar.Size = UDim2.new(1, 0, 0, 30)
		self.LoadTitleBar.Position = UDim2.new(0, 0, 0, 0)
		self.LoadTitleBar.BackgroundColor3 = self.Colors.CollapseBtnColor
		self.LoadTitleBar.BorderSizePixel = 0
		self.LoadTitleBar.ZIndex = 2
		self.LoadTitleBar.Parent = self.LoadFrame

		local keytitleCorner = Instance.new("UICorner")
		keytitleCorner.CornerRadius = UDim.new(0, self.Config.CornerRadius)
		keytitleCorner.Parent = self.KeyTitleBar

		self.LoadTitleText = Instance.new("TextLabel")
		self.LoadTitleText.Name = "LoadTitleText"
		self.LoadTitleText.Size = UDim2.new(1, 0, 1, 0)
		self.LoadTitleText.Position = UDim2.new(0, 0, 0, 0)
		self.LoadTitleText.BackgroundTransparency = 1
		self.LoadTitleText.Text = config.Title
		self.LoadTitleText.TextColor3 = self.Colors.TitleColor
		self.LoadTitleText.TextXAlignment = Enum.TextXAlignment.Center
		self.LoadTitleText.Font = Enum.Font.GothamBold
		self.LoadTitleText.TextSize = 14
		self.LoadTitleText.ZIndex = 3
		self.LoadTitleText.Parent = self.LoadTitleBar

		self.DescriptionText = Instance.new("TextLabel")
		self.DescriptionText.Name = "DescriptionText"
		self.DescriptionText.Size = UDim2.new(0, 154, 0, 21)
		self.DescriptionText.Position = UDim2.new(0.09, 0, 0.746, 0)
		self.DescriptionText.BackgroundTransparency = 1
		self.DescriptionText.Text = config.Description
		self.DescriptionText.TextColor3 = self.Colors.TitleColor
		self.DescriptionText.TextXAlignment = Enum.TextXAlignment.Center
		self.DescriptionText.Font = Enum.Font.GothamBold
		self.DescriptionText.TextScaled = true
		self.DescriptionText.ZIndex = 3
		self.DescriptionText.Parent = self.LoadFrame

		self.LoadingBarFrame = Instance.new("CanvasGroup")
		self.LoadingBarFrame.Name = "LoadingBar"
		self.LoadingBarFrame.Size = UDim2.new(0, 163, 0, 27)
		self.LoadingBarFrame.Position = UDim2.new(0.063, 0, 0.429, 0)
		self.LoadingBarFrame.BackgroundColor3 = Color3.fromRGB(27, 27, 27)
		self.LoadingBarFrame.Parent = self.LoadFrame

		if config.UIStrokeEnabled then
			local uiStroke = Instance.new("UIStroke")
			uiStroke.Thickness = config.UIStrokeThickness
			uiStroke.Color = self.Colors.UIStrokeColor
			uiStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
			uiStroke.Parent = self.LoadingBarFrame
		end

		self.LoadingBarCorner = Instance.new("UICorner")
		self.LoadingBarCorner.CornerRadius = UDim.new(0, 8)
		self.LoadingBarCorner.Parent = self.LoadingBarFrame

		self.LoadingBar = Instance.new("Frame")
		self.LoadingBar.Name = "LoadingBar"
		self.LoadingBar.Size = UDim2.new(0, 0, 1, 0)
		self.LoadingBar.Position = UDim2.new(0, 0, 0, 0)
		self.LoadingBar.BackgroundColor3 = self.Colors.LoadingBarColor
		self.LoadingBar.Parent = self.LoadingBarFrame

		task.spawn(function()
			local currentPercent = 0
			local random = Random.new()

			while currentPercent < 1 do
				local increment = random:NextNumber(0.05, 0.2)
				currentPercent = math.min(currentPercent + increment, 1)

				local moveDuration = random:NextNumber(config.WaitTime[1], config.WaitTime[2])

				local tween = TS:Create(self.LoadingBar, TweenInfo.new(moveDuration, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
					Size = UDim2.new(currentPercent, 0, 1, 0)
				})

				tween:Play()
				tween.Completed:Wait()

				if currentPercent < 1 then
					task.wait(random:NextNumber(config.WaitTime2[1], config.WaitTime2[2]))
				end

				task.wait()
			end

			task.wait(0.5)

			TS:Create(self.LoadFrame, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
				Size = UDim2.new(0, 0, 0, 0),
				BackgroundTransparency = 1
			}):Play()

			task.wait(1)

			config.CallBack()
			self:CreateUI()
			self.LoadFrame:Destroy()
		end)
	end,

	ToggleMinimize = function(self)
		self.Minimized = not self.Minimized
		if self.Minimized then
			self.MainFrame.Size = UDim2.new(0, self.OriginalSize.X.Offset, 0, self.Config.TitleHeight)
			self.MinimizeButton.Text = "+"
			self.ScrollingFrame.Visible = false
		else
			self.MainFrame.Size = self.OriginalSize
			self.MinimizeButton.Text = "-"
			self.ScrollingFrame.Visible = true
		end
	end,

	ToggleFrameVisibility = function(self)
		self.FVisible = not self.FVisible
		self.UIWrapper.Visible = self.FVisible
	end,

	ToggleGuiVisibility = function(self)
		self.GVisible = not self.GVisible
		self.ScreenGui.Enabled = self.GVisible
	end,

	SetTitle = function(self, newTitle)
		self.Config.TitleText = newTitle
		self.TitleText.Text = newTitle
	end,

	SetTheme = function(self, newTheme) -- doesnt work rn
		for colorName, Color in pairs(UILibrary.DefaultThemes[newTheme]) do
			self.Colors[colorName] = Color
		end
	end,

	MakeDraggableButton = function(self, config)
		local dragImage = Instance.new("ImageButton")
		dragImage.Name = "DragImage"
		dragImage.Size = config.Size
		dragImage.Position = config.Position
		dragImage.BackgroundTransparency = 1
		dragImage.Image = utils:GetImage({Name = config.Name, Url = config.URL})
		dragImage.ScaleType = Enum.ScaleType.Fit
		dragImage.ZIndex = 2
		dragImage.Parent = self.ScreenGui

		if config.UIStrokeEnabled then
			local uistroke = Instance.new("UIStroke")
			uistroke.Thickness = config.UIStrokeThickness
			uistroke.Color = self.Colors.UIStrokeColor
			uistroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
			uistroke.Parent = dragImage
		end

		if config.BGColor then
			dragImage.BackgroundColor3 = config.BGColor
		end

		if config.ScaleType then
			dragImage.ScaleType = config.ScaleType
		end

		if config.BGTransparency then
			dragImage.BackgroundTransparency = config.BGTransparency
		end

		local dragCorner = Instance.new("UICorner")
		dragCorner.CornerRadius = config.CornerRadius
		dragCorner.Parent = dragImage

		utils:MakeDraggable({Frame = dragImage})

		dragImage.MouseButton1Click:Connect(function()
			config.Callback()
		end)
	end,

	AddSection = function(self, text)
		local section = Instance.new("Frame")
		section.Name = "Section_" .. text
		section.Size = UDim2.new(1, -24, 0, self.Config.SectionHeight)
		section.BackgroundTransparency = 1
		section.LayoutOrder = #self.Elements + 1
		section.Parent = self.ScrollingFrame

		local sectionText = Instance.new("TextLabel")
		sectionText.Name = "TextLabel"
		sectionText.Size = UDim2.new(1, 0, 1, 0)
		sectionText.BackgroundTransparency = 1
		sectionText.Text = text
		sectionText.TextColor3 = self.Colors.SectionColor
		sectionText.TextXAlignment = Enum.TextXAlignment.Left
		sectionText.Font = Enum.Font.GothamBold
		sectionText.TextSize = self.Config.TextSize + 1
		sectionText.Parent = section

		local tab = Instance.new("CanvasGroup")
		tab.Name = "Tab_" .. text
		tab.Size = UDim2.new(0, 16, 0, 60) 
		tab.BackgroundColor3 = self.Colors.TabColor
		tab.GroupTransparency = 0.3
		tab.LayoutOrder = #self.Tabs + 1
		tab.Parent = self.TabScrollingFrame

		local corner = Instance.new("UICorner")
		corner.CornerRadius = UDim.new(0, 4)
		corner.Parent = tab

		local accent = Instance.new("Frame")
		accent.Name = "Accent"
		accent.Size = UDim2.new(0, 2, 0, 0)
		accent.Position = UDim2.new(1, -2, 0.5, 0)
		accent.AnchorPoint = Vector2.new(0, 0.5)
		accent.BackgroundColor3 = self.Colors.AccentColor
		accent.BorderSizePixel = 0
		accent.Parent = tab

		local btn = Instance.new("TextButton")
		btn.Name = "Button_" .. text
		btn.Rotation = 90
		btn.Size = UDim2.new(0, 56, 0, 16)
		btn.Position = UDim2.new(0.5, 0, 0.5, 0)
		btn.AnchorPoint = Vector2.new(0.5, 0.5)
		btn.BackgroundTransparency = 1
		btn.Text = text:upper()
		btn.TextColor3 = self.Colors.TitleColor
		btn.TextTransparency = 0.2
		btn.Font = Enum.Font.GothamBold
		btn.TextScaled = true
		btn.Parent = tab

		tab.MouseEnter:Connect(function()
			TS:Create(tab, TweenInfo.new(0.3), {GroupTransparency = 0, BackgroundColor3 = self.Colors.AccentColor:Lerp(self.Colors.TabColor, 0.8)}):Play()
			TS:Create(btn, TweenInfo.new(0.3), {TextTransparency = 0}):Play()
		end)

		tab.MouseLeave:Connect(function()
			TS:Create(tab, TweenInfo.new(0.3), {GroupTransparency = 0.3, BackgroundColor3 = self.Colors.TabColor}):Play()
			TS:Create(btn, TweenInfo.new(0.3), {TextTransparency = 0.2}):Play()
		end)

		btn.MouseButton1Click:Connect(function()
			for _, t in pairs(self.Tabs) do
				TS:Create(t.Accent, TweenInfo.new(0.3), {Size = UDim2.new(0, 2, 0, 0)}):Play()
			end

			TS:Create(accent, TweenInfo.new(0.3, Enum.EasingStyle.Back), {Size = UDim2.new(0, 2, 0, 40)}):Play()

			local relativeOffset = section.AbsolutePosition.Y - self.ScrollingFrame.AbsolutePosition.Y
			local targetY = self.ScrollingFrame.CanvasPosition.Y + relativeOffset

			TS:Create(self.ScrollingFrame, TweenInfo.new(0.5, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
				CanvasPosition = Vector2.new(0, targetY)
			}):Play()
		end)

		section:SetAttribute("TabName", tab.Name)

		table.insert(self.Sections, section)
		table.insert(self.Elements, section)
		table.insert(self.Tabs, tab)

		return section, tab
	end,

	AddLabel = function(self, text)
		local label = Instance.new("TextLabel")
		label.Name = "Label_" .. text
		label.Size = UDim2.new(1, -24, 0, 18)
		label.Position = UDim2.new(0, 12, 0, 0)
		label.BackgroundTransparency = 1
		label.Text = text
		label.TextColor3 = self.Colors.LabelColor
		label.TextXAlignment = Enum.TextXAlignment.Left
		label.Font = self.Config.Font
		label.TextSize = self.Config.TextSize
		label.LayoutOrder = #self.Elements + 1
		label.Parent = self.ScrollingFrame

		table.insert(self.Elements, label)
		return label
	end,

	AddButton = function(self, config)
		local button = Instance.new("TextButton")
		button.Name = "Button_" .. config.Text
		button.Size = UDim2.new(1, -24, 0, 30)
		button.Position = UDim2.new(0, 12, 0, 0)
		button.BackgroundColor3 = self.Colors.ButtonColor
		button.BorderSizePixel = 0
		button.Text = config.Text
		button.TextColor3 = self.Colors.TitleColor
		button.Font = self.Config.Font
		button.TextSize = tonumber(config.TextSize) or self.Config.TextSize
		button.LayoutOrder = #self.Elements + 1
		button.AutoButtonColor = false
		button.Parent = self.ScrollingFrame

		local corner = Instance.new("UICorner")
		corner.CornerRadius = UDim.new(0, 4)
		corner.Parent = button

		if self.Config.UIStrokeEnabled then
			local buttonStroke = Instance.new("UIStroke")
			buttonStroke.Thickness = self.Config.UIStrokeThickness
			buttonStroke.Color = self.Colors.UIStrokeColor
			buttonStroke.Parent = button
		end

		button.MouseEnter:Connect(function()
			button.BackgroundColor3 = self.Colors.ButtonHoverColor
		end)

		button.MouseLeave:Connect(function()
			button.BackgroundColor3 = self.Colors.ButtonColor
		end)

		if config.Callback then
			button.MouseButton1Click:Connect(function()
				local originalSize = button.Size
				button.Size = UDim2.new(originalSize.X.Scale, originalSize.X.Offset, originalSize.Y.Scale, originalSize.Y.Offset - 2)
				task.wait(0.08)
				button.Size = originalSize

				config.Callback()
			end)
		end

		table.insert(self.Elements, button)
		return button
	end,

	AddToggle = function(self, config)
		local lib = self

		local toggleFrame = Instance.new("Frame")
		toggleFrame.Name = "Toggle_" .. config.Text
		toggleFrame.Size = UDim2.new(1, -24, 0, 32)
		toggleFrame.Position = UDim2.new(0, 12, 0, 0)
		toggleFrame.BackgroundTransparency = 1
		toggleFrame.LayoutOrder = #self.Elements + 1
		toggleFrame.Parent = self.ScrollingFrame

		local toggleText = Instance.new("TextLabel")
		toggleText.Name = "TextLabel"
		toggleText.Size = UDim2.new(0.65, 0, 1, 0)
		toggleText.Position = UDim2.new(0, 0, 0, 0)
		toggleText.BackgroundTransparency = 1
		toggleText.Text = config.Text
		toggleText.TextColor3 = self.Colors.TitleColor
		toggleText.TextXAlignment = Enum.TextXAlignment.Left
		toggleText.Font = self.Config.Font
		toggleText.TextSize = self.Config.TextSize
		toggleText.Parent = toggleFrame

		local toggleButton = Instance.new("TextButton")
		toggleButton.Name = "ToggleButton"
		toggleButton.Size = UDim2.new(0.3, 0, 0.75, 0)
		toggleButton.Position = UDim2.new(0.68, 0, 0.125, 0)
		toggleButton.BackgroundColor3 = self.Colors.ToggleColor
		toggleButton.BorderSizePixel = 0
		toggleButton.Text = config.Default and "ON" or "OFF"
		toggleButton.TextColor3 = config.Default and self.Colors.ToggleColorON or self.Colors.ToggleColorOFF
		toggleButton.Font = self.Config.Font
		toggleButton.TextSize = self.Config.TextSize
		toggleButton.AutoButtonColor = false
		toggleButton.Parent = toggleFrame

		local corner = Instance.new("UICorner")
		corner.CornerRadius = UDim.new(0, 12)
		corner.Parent = toggleButton

		if self.Config.UIStrokeEnabled then
			local buttonStroke = Instance.new("UIStroke")
			buttonStroke.Thickness = self.Config.UIStrokeThickness
			buttonStroke.Color = self.Colors.UIStrokeColor
			buttonStroke.Parent = toggleButton
		end

		local toggle = {}

		local flag = config.Flag

		local state = config.Default or false

		if self.UserConfig[flag] ~= nil then
			state = self.UserConfig[flag]
		end

		local function update(value)
			state = value
			toggleButton.Text = state and "ON" or "OFF"
			toggleButton.TextColor3 = state and lib.Colors.ToggleColorON or lib.Colors.ToggleColorOFF

			if config.SavingEnabled then
				lib.UserConfig[flag] = state
			end

			if config.Callback then
				config.Callback(state)
			end
		end

		update(state)

		function toggle:Set(bool)
			update(bool)
		end

		toggleButton.MouseEnter:Connect(function()
			toggleButton.BackgroundColor3 = Color3.new(
				self.Colors.ToggleColor.R * 1.1,
				self.Colors.ToggleColor.G * 1.1,
				self.Colors.ToggleColor.B * 1.1
			)
		end)

		toggleButton.MouseLeave:Connect(function()
			toggleButton.BackgroundColor3 = self.Colors.ToggleColor
		end)

		toggleButton.MouseButton1Click:Connect(function()
			update(not state)
		end)

		table.insert(self.Elements, toggleFrame)
		return toggle
	end,
	
	AddSettingToggle = function(self, config)

		local lib = self

		local toggleFrame = Instance.new("Frame")
		toggleFrame.Name = "Toggle_" .. config.Text
		toggleFrame.Size = UDim2.new(1, -12, 0, 32)
		toggleFrame.Position = UDim2.new(0, 12, 0, 0)
		toggleFrame.BackgroundTransparency = 1
		toggleFrame.LayoutOrder = #self.SettingElements + 1
		toggleFrame.Parent = self.SettingScrollingFrame

		local toggleText = Instance.new("TextLabel")
		toggleText.Name = "TextLabel"
		toggleText.Size = UDim2.new(0.65, 0, 1, 0)
		toggleText.Position = UDim2.new(0, 0, 0, 0)
		toggleText.BackgroundTransparency = 1
		toggleText.Text = config.Text
		toggleText.TextColor3 = self.Colors.TitleColor
		toggleText.TextXAlignment = Enum.TextXAlignment.Left
		toggleText.Font = self.Config.Font
		toggleText.TextSize = self.Config.TextSize
		toggleText.Parent = toggleFrame

		local toggleButton = Instance.new("TextButton")
		toggleButton.Name = "ToggleButton"
		toggleButton.Size = UDim2.new(0.3, 0, 0.75, 0)
		toggleButton.Position = UDim2.new(0.9, 0, 0.125, 0)
		toggleButton.BackgroundColor3 = self.Colors.ToggleColor
		toggleButton.BorderSizePixel = 0
		toggleButton.Text = config.Default and "ON" or "OFF"
		toggleButton.TextColor3 = config.Default and self.Colors.ToggleColorON or self.Colors.ToggleColorOFF
		toggleButton.Font = self.Config.Font
		toggleButton.TextSize = self.Config.TextSize
		toggleButton.AutoButtonColor = false
		toggleButton.Parent = toggleFrame

		local corner = Instance.new("UICorner")
		corner.CornerRadius = UDim.new(0, 12)
		corner.Parent = toggleButton

		if self.Config.UIStrokeEnabled then
			local buttonStroke = Instance.new("UIStroke")
			buttonStroke.Thickness = self.Config.UIStrokeThickness
			buttonStroke.Color = self.Colors.UIStrokeColor
			buttonStroke.Parent = toggleButton
		end

		local toggle = {}

		local flag = config.Flag

		local state = config.Default or false

		if self.UserSettings[flag] ~= nil then
			state = self.UserSettings[flag]
		end

		local function update(value)

			state = value
			toggleButton.Text = state and "ON" or "OFF"
			toggleButton.TextColor3 = state and lib.Colors.ToggleColorON or lib.Colors.ToggleColorOFF

			if config.SavingEnabled then
				lib.UserSettings[flag] = state
			end

			if config.Callback then
				config.Callback(state)
			end

		end

		update(state)

		function toggle:Set(bool)
			update(bool)
		end

		toggleButton.MouseEnter:Connect(function()
			toggleButton.BackgroundColor3 = Color3.new(
				self.Colors.ToggleColor.R * 1.1,
				self.Colors.ToggleColor.G * 1.1,
				self.Colors.ToggleColor.B * 1.1
			)
		end)

		toggleButton.MouseLeave:Connect(function()
			toggleButton.BackgroundColor3 = self.Colors.ToggleColor
		end)

		toggleButton.MouseButton1Click:Connect(function()
			update(not state)
		end)

		table.insert(self.SettingElements, toggleFrame)
		return toggle

	end,

	AddSettingDropdown = function(self, config)

		local dropdownHeight = config.DropDownHeight or 44
		local optionHeight = config.OptionHeight or 30
		local isOpen = false
		local dropdown = {}

		local lib = self

		local dropdownFrame = Instance.new("Frame")
		dropdownFrame.Name = "Dropdown_" .. config.Text
		dropdownFrame.Size = UDim2.new(1, -12, 0, dropdownHeight)
		dropdownFrame.Position = UDim2.new(0, 12, 0, 0)
		dropdownFrame.BackgroundTransparency = 1
		dropdownFrame.ClipsDescendants = true
		dropdownFrame.LayoutOrder = #self.SettingElements + 1
		dropdownFrame.Parent = self.SettingScrollingFrame

		local headerBtn = Instance.new("TextButton")
		headerBtn.Name = "Header"
		headerBtn.Size = UDim2.new(1, 0, 0, dropdownHeight)
		headerBtn.BackgroundColor3 = self.Colors.ButtonColor
		headerBtn.Text = ""
		headerBtn.AutoButtonColor = false
		headerBtn.Parent = dropdownFrame

		local headerCorner = Instance.new("UICorner")
		headerCorner.CornerRadius = UDim.new(0, 6)
		headerCorner.Parent = headerBtn

		if self.Config.UIStrokeEnabled then
			local stroke = Instance.new("UIStroke")
			stroke.Thickness = self.Config.UIStrokeThickness
			stroke.Color = self.Colors.UIStrokeColor
			stroke.Parent = headerBtn
		end

		local titleLabel = Instance.new("TextLabel")
		titleLabel.Name = "Title"
		titleLabel.Size = UDim2.new(1, -40, 0, 20)
		titleLabel.Position = UDim2.new(0, 10, 0, 5)
		titleLabel.BackgroundTransparency = 1
		titleLabel.Text = config.Text
		titleLabel.TextColor3 = self.Colors.TitleColor
		titleLabel.TextXAlignment = Enum.TextXAlignment.Left
		titleLabel.Font = self.Config.Font
		titleLabel.TextSize = self.Config.TextSize
		titleLabel.Parent = headerBtn

		local selectedLabel = Instance.new("TextLabel")
		selectedLabel.Name = "Selected"
		selectedLabel.Size = UDim2.new(1, -40, 0, 18)
		selectedLabel.Position = UDim2.new(0, 10, 0, 22)
		selectedLabel.BackgroundTransparency = 1
		selectedLabel.Text = config.Default or "None"
		selectedLabel.TextColor3 = self.Colors.AccentColor
		selectedLabel.TextXAlignment = Enum.TextXAlignment.Left
		selectedLabel.Font = self.Config.Font
		selectedLabel.TextSize = self.Config.TextSize - 1
		selectedLabel.Parent = headerBtn

		local arrow = Instance.new("ImageLabel")
		arrow.Name = "Arrow"
		arrow.Size = UDim2.new(0, 20, 0, 20)
		arrow.Position = UDim2.new(1, -25, 0.5, 0)
		arrow.AnchorPoint = Vector2.new(0, 0.5)
		arrow.BackgroundTransparency = 1
		arrow.Image = "rbxassetid://6034818372"
		arrow.ImageColor3 = self.Colors.LabelColor
		arrow.Parent = headerBtn

		local optionHolder = Instance.new("Frame")
		optionHolder.Name = "OptionHolder"
		optionHolder.Size = UDim2.new(1, 0, 0, 0)
		optionHolder.Position = UDim2.new(0, 0, 0, dropdownHeight + 2)
		optionHolder.BackgroundTransparency = 1
		optionHolder.Parent = dropdownFrame

		local optionList = Instance.new("UIListLayout")
		optionList.Padding = UDim.new(0, 2)
		optionList.SortOrder = Enum.SortOrder.LayoutOrder
		optionList.Parent = optionHolder

		local function toggle()

			isOpen = not isOpen

			local count = #config.Options
			local newHeight = isOpen and (dropdownHeight + (count * optionHeight) + (count * 2) + 5) or dropdownHeight

			TS:Create(dropdownFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Size = UDim2.new(1, -12, 0, newHeight)
			}):Play()

			TS:Create(arrow, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Rotation = isOpen and 180 or 0
			}):Play()

			TS:Create(headerBtn, TweenInfo.new(0.3), {
				BackgroundColor3 = isOpen and self.Colors.ButtonHoverColor or self.Colors.ButtonColor
			}):Play()

		end

		local function update(options, clearcurrent)

			for _, child in pairs(optionHolder:GetChildren()) do
				if child:IsA("TextButton") then
					child:Destroy()
				end
			end

			local selected = nil

			local currentOptions = options or {}

			if clearcurrent then
				selectedLabel.Text = "None"
				selected = nil
			end

			local flag = config.Flag

			if lib.UserSettings[flag] ~= nil then
				selected = lib.UserSettings[flag]
				selectedLabel.Text = tostring(selected)
			end

			for i, option in ipairs(currentOptions) do

				local optBtn = Instance.new("TextButton")
				optBtn.Name = option
				optBtn.Size = UDim2.new(1, 0, 0, optionHeight)
				optBtn.BackgroundColor3 = lib.Colors.CollapseBtnColor
				optBtn.Text = option
				optBtn.TextColor3 = lib.Colors.LabelColor
				optBtn.Font = lib.Config.Font
				optBtn.TextSize = lib.Config.TextSize
				optBtn.AutoButtonColor = false
				optBtn.Parent = optionHolder

				local optCorner = Instance.new("UICorner")
				optCorner.CornerRadius = UDim.new(0, 4)
				optCorner.Parent = optBtn

				optBtn.MouseButton1Click:Connect(function()

					selectedLabel.Text = option
					selected = option

					if config.SavingEnabled then
						lib.UserSettings[flag] = selected
					end

					toggle()

					if config.Callback then
						config.Callback(option)
					end

				end)

				optBtn.MouseEnter:Connect(function()

					TS:Create(optBtn, TweenInfo.new(0.2), {
						BackgroundColor3 = self.Colors.AccentColor,
						TextColor3 = Color3.new(1, 1, 1)
					}):Play()

				end)

				optBtn.MouseLeave:Connect(function()

					TS:Create(optBtn, TweenInfo.new(0.2), {
						BackgroundColor3 = self.Colors.CollapseBtnColor,
						TextColor3 = self.Colors.LabelColor
					}):Play()

				end)

			end

		end

		update(config.Options, true)

		function dropdown:Update(options)
			update(options, true)
		end

		function dropdown:Set(option)

			selectedLabel.Text = tostring(option)

			if config.SavingEnabled and config.Flag then
				lib.UserSettings[config.Flag] = option
			end

			if config.Callback then
				task.spawn(config.Callback, option)
			end

			if isOpen then
				toggle()
			end

		end

		headerBtn.MouseButton1Click:Connect(toggle)

		table.insert(self.SettingElements, dropdownFrame)
		return dropdown
	end,
	
	AddSettingMultiDropdown = function(self, config)

		local dropdownHeight = config.DropDownHeight or 44
		local optionHeight = config.OptionHeight or 30
		local isOpen = false
		local dropdown = {}
		local selectedItems = {}

		local lib = self

		local dropdownFrame = Instance.new("Frame")
		dropdownFrame.Name = "MultiDropdown_" .. config.Text
		dropdownFrame.Size = UDim2.new(1, -12, 0, dropdownHeight)
		dropdownFrame.Position = UDim2.new(0, 12, 0, 0)
		dropdownFrame.BackgroundTransparency = 1
		dropdownFrame.ClipsDescendants = true
		dropdownFrame.LayoutOrder = #self.SettingElements + 1
		dropdownFrame.Parent = self.SettingScrollingFrame

		local headerBtn = Instance.new("TextButton")
		headerBtn.Name = "Header"
		headerBtn.Size = UDim2.new(1, 0, 0, dropdownHeight)
		headerBtn.BackgroundColor3 = self.Colors.ButtonColor
		headerBtn.Text = ""
		headerBtn.AutoButtonColor = false
		headerBtn.Parent = dropdownFrame

		local headerCorner = Instance.new("UICorner")
		headerCorner.CornerRadius = UDim.new(0, 6)
		headerCorner.Parent = headerBtn

		if self.Config.UIStrokeEnabled then
			local stroke = Instance.new("UIStroke")
			stroke.Thickness = self.Config.UIStrokeThickness
			stroke.Color = self.Colors.UIStrokeColor
			stroke.Parent = headerBtn
		end

		local titleLabel = Instance.new("TextLabel")
		titleLabel.Name = "Title"
		titleLabel.Size = UDim2.new(1, -40, 0, 20)
		titleLabel.Position = UDim2.new(0, 10, 0, 5)
		titleLabel.BackgroundTransparency = 1
		titleLabel.Text = config.Text
		titleLabel.TextColor3 = self.Colors.TitleColor
		titleLabel.TextXAlignment = Enum.TextXAlignment.Left
		titleLabel.Font = self.Config.Font
		titleLabel.TextSize = self.Config.TextSize
		titleLabel.Parent = headerBtn

		local selectedLabel = Instance.new("TextLabel")
		selectedLabel.Name = "Selected"
		selectedLabel.Size = UDim2.new(1, -40, 0, 18)
		selectedLabel.Position = UDim2.new(0, 10, 0, 22)
		selectedLabel.BackgroundTransparency = 1
		selectedLabel.Text = "None"
		selectedLabel.TextColor3 = self.Colors.AccentColor
		selectedLabel.TextXAlignment = Enum.TextXAlignment.Left
		selectedLabel.Font = self.Config.Font
		selectedLabel.TextScaled = true
		selectedLabel.Parent = headerBtn

		local sizeConstraint = Instance.new("UITextSizeConstraint")
		sizeConstraint.MaxTextSize = self.Config.TextSize - 1
		sizeConstraint.Parent = selectedLabel

		local arrow = Instance.new("ImageLabel")
		arrow.Name = "Arrow"
		arrow.Size = UDim2.new(0, 20, 0, 20)
		arrow.Position = UDim2.new(1, -25, 0.5, 0)
		arrow.AnchorPoint = Vector2.new(0, 0.5)
		arrow.BackgroundTransparency = 1
		arrow.Image = "rbxassetid://6034818372"
		arrow.ImageColor3 = self.Colors.LabelColor
		arrow.Parent = headerBtn

		local optionHolder = Instance.new("Frame")
		optionHolder.Name = "OptionHolder"
		optionHolder.Size = UDim2.new(1, 0, 0, 0)
		optionHolder.Position = UDim2.new(0, 0, 0, dropdownHeight + 2)
		optionHolder.BackgroundTransparency = 1
		optionHolder.Parent = dropdownFrame

		local optionList = Instance.new("UIListLayout")
		optionList.Padding = UDim.new(0, 2)
		optionList.SortOrder = Enum.SortOrder.LayoutOrder
		optionList.Parent = optionHolder

		local function updateHeaderText()

			if #selectedItems == 0 then
				selectedLabel.Text = "None"
			else
				selectedLabel.Text = table.concat(selectedItems, ", ")
			end

		end

		local function toggle()

			isOpen = not isOpen

			local count = #config.Options
			local newHeight = isOpen and (dropdownHeight + (count * optionHeight) + (count * 2) + 5) or dropdownHeight

			TS:Create(dropdownFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Size = UDim2.new(1, -12, 0, newHeight)
			}):Play()

			TS:Create(arrow, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Rotation = isOpen and 180 or 0
			}):Play()

			TS:Create(headerBtn, TweenInfo.new(0.3), {
				BackgroundColor3 = isOpen and self.Colors.ButtonHoverColor or self.Colors.ButtonColor
			}):Play()

		end

		local function update(options, clearcurrent)

			for _, child in pairs(optionHolder:GetChildren()) do
				if child:IsA("TextButton") then
					child:Destroy()
				end
			end

			local currentOptions = options or {}

			if clearcurrent then
				selectedItems = {}
				updateHeaderText()
			end

			local flag = config.Flag

			if lib.UserSettings[flag] ~= nil and type(lib.UserSettings[flag]) == "table" then
				selectedItems = lib.UserSettings[flag]
				updateHeaderText()
			elseif config.Default then
				if type(config.Default) == "table" then
					selectedItems = config.Default
				else
					selectedItems = { config.Default }
				end
				updateHeaderText()
			end

			for i, option in ipairs(currentOptions) do

				local optBtn = Instance.new("TextButton")
				optBtn.Name = option
				optBtn.Size = UDim2.new(1, 0, 0, optionHeight)

				local isSelected = table.find(selectedItems, option)
				optBtn.BackgroundColor3 = isSelected and self.Colors.AccentColor or lib.Colors.CollapseBtnColor
				optBtn.TextColor3 = isSelected and Color3.new(1, 1, 1) or lib.Colors.LabelColor

				optBtn.Text = option
				optBtn.Font = lib.Config.Font
				optBtn.TextSize = lib.Config.TextSize
				optBtn.AutoButtonColor = false
				optBtn.Parent = optionHolder

				local optCorner = Instance.new("UICorner")
				optCorner.CornerRadius = UDim.new(0, 4)
				optCorner.Parent = optBtn

				optBtn.MouseButton1Click:Connect(function()

					local index = table.find(selectedItems, option)

					if index then

						table.remove(selectedItems, index)

						TS:Create(optBtn, TweenInfo.new(0.2), {
							BackgroundColor3 = lib.Colors.CollapseBtnColor,
							TextColor3 = lib.Colors.LabelColor
						}):Play()

					else

						table.insert(selectedItems, option)

						TS:Create(optBtn, TweenInfo.new(0.2), {
							BackgroundColor3 = self.Colors.AccentColor,
							TextColor3 = Color3.new(1, 1, 1)
						}):Play()

					end

					updateHeaderText()

					if config.SavingEnabled then
						lib.UserSettings[flag] = selectedItems
					end

					if config.Callback then
						config.Callback(selectedItems)
					end

				end)

			end

		end

		update(config.Options, true)

		function dropdown:Update(config)
			update(config.Options, true)
		end

		function dropdown:Set(newList)

			selectedItems = type(newList) == "table" and newList or { newList }

			updateHeaderText()

			if config.SavingEnabled and config.Flag then
				lib.UserSettings[config.Flag] = selectedItems
			end

			for _, btn in pairs(optionHolder:GetChildren()) do
				if btn:IsA("TextButton") then

					local isSelected = table.find(selectedItems, btn.Name)

					TS:Create(btn, TweenInfo.new(0.2), {
						BackgroundColor3 = isSelected and self.Colors.AccentColor or lib.Colors.CollapseBtnColor,
						TextColor3 = isSelected and Color3.new(1, 1, 1) or lib.Colors.LabelColor
					}):Play()

				end
			end

			if config.Callback then
				task.spawn(config.Callback, selectedItems)
			end

		end

		headerBtn.MouseButton1Click:Connect(toggle)

		table.insert(self.SettingElements, dropdownFrame)
		return dropdown

	end,
	
	AddSettingButton = function(self, config)

		local button = Instance.new("TextButton")
		button.Name = "Button_" .. config.Text
		button.Size = UDim2.new(1, -12, 0, 30)
		button.Position = UDim2.new(0, 12, 0, 0)
		button.BackgroundColor3 = self.Colors.ButtonColor
		button.BorderSizePixel = 0
		button.Text = config.Text
		button.TextColor3 = self.Colors.TitleColor
		button.Font = self.Config.Font
		button.TextSize = tonumber(config.TextSize) or self.Config.TextSize
		button.LayoutOrder = #self.SettingElements + 1
		button.AutoButtonColor = false
		button.Parent = self.SettingScrollingFrame

		local corner = Instance.new("UICorner")
		corner.CornerRadius = UDim.new(0, 4)
		corner.Parent = button

		if self.Config.UIStrokeEnabled then
			local buttonStroke = Instance.new("UIStroke")
			buttonStroke.Thickness = self.Config.UIStrokeThickness
			buttonStroke.Color = self.Colors.UIStrokeColor
			buttonStroke.Parent = button
		end

		button.MouseEnter:Connect(function()
			button.BackgroundColor3 = self.Colors.ButtonHoverColor
		end)

		button.MouseLeave:Connect(function()
			button.BackgroundColor3 = self.Colors.ButtonColor
		end)

		if config.Callback then
			button.MouseButton1Click:Connect(function()

				local originalSize = button.Size
				button.Size = UDim2.new(originalSize.X.Scale, originalSize.X.Offset, originalSize.Y.Scale, originalSize.Y.Offset - 2)
				task.wait(0.08)
				button.Size = originalSize

				config.Callback()

			end)
		end

		table.insert(self.SettingElements, button)
		return button

	end,


	AddSettingSlider = function(self, config)

		local lib = self
		local slider = {}

		local dragging = false
		local currentValue = config.Default or config.Min
		local flag = config.Flag

		local sliderFrame = Instance.new("Frame")
		sliderFrame.Name = "Slider_" .. config.Text
		sliderFrame.Size = UDim2.new(1, -12, 0, 50)
		sliderFrame.Position = UDim2.new(0, 12, 0, 0)
		sliderFrame.BackgroundTransparency = 1
		sliderFrame.LayoutOrder = #self.SettingElements + 1
		sliderFrame.Parent = self.SettingScrollingFrame

		local title = Instance.new("TextLabel")
		title.Name = "Title"
		title.Size = UDim2.new(1, -60, 0, 20)
		title.Position = UDim2.new(0, 0, 0, 0)
		title.BackgroundTransparency = 1
		title.Text = config.Text
		title.TextColor3 = self.Colors.TitleColor
		title.Font = self.Config.Font
		title.TextSize = self.Config.TextSize
		title.TextXAlignment = Enum.TextXAlignment.Left
		title.Parent = sliderFrame

		local valueInput = Instance.new("TextBox")
		valueInput.Name = "ValueInput"
		valueInput.Size = UDim2.new(0, 50, 0, 20)
		valueInput.Position = UDim2.new(1, 0, 0, 0)
		valueInput.AnchorPoint = Vector2.new(1, 0)
		valueInput.BackgroundTransparency = 0.1
		valueInput.BackgroundColor3 = self.Colors.ButtonColor
		valueInput.Text = tostring(currentValue)
		valueInput.TextColor3 = self.Colors.AccentColor
		valueInput.Font = self.Config.Font
		valueInput.TextSize = self.Config.TextSize - 2
		valueInput.ClearTextOnFocus = false
		valueInput.Parent = sliderFrame
		Instance.new("UICorner", valueInput).CornerRadius = UDim.new(0, 4)

		local sliderTrack = Instance.new("TextButton")
		sliderTrack.Name = "Track"
		sliderTrack.Text = ""
		sliderTrack.Size = UDim2.new(1, 0, 0, 6)
		sliderTrack.Position = UDim2.new(0, 0, 0, 32)
		sliderTrack.BackgroundColor3 = self.Colors.SliderColor
		sliderTrack.BorderSizePixel = 0
		sliderTrack.AutoButtonColor = false
		sliderTrack.Parent = sliderFrame
		Instance.new("UICorner", sliderTrack).CornerRadius = UDim.new(1, 0)

		local sliderFill = Instance.new("Frame")
		sliderFill.Name = "Fill"
		sliderFill.Size = UDim2.new(0, 0, 1, 0)
		sliderFill.BackgroundColor3 = self.Colors.AccentColor
		sliderFill.BorderSizePixel = 0
		sliderFill.Parent = sliderTrack
		Instance.new("UICorner", sliderFill).CornerRadius = UDim.new(1, 0)

		local sliderHandle = Instance.new("Frame")
		sliderHandle.Name = "Handle"
		sliderHandle.Size = UDim2.new(0, 14, 0, 14)
		sliderHandle.AnchorPoint = Vector2.new(0.5, 0.5)
		sliderHandle.Position = UDim2.new(0, 0, 0.5, 0)
		sliderHandle.BackgroundColor3 = self.Colors.SliderHandleColor
		sliderHandle.Parent = sliderTrack
		Instance.new("UICorner", sliderHandle).CornerRadius = UDim.new(1, 0)

		if self.Config.UIStrokeEnabled then
			local handleStroke = Instance.new("UIStroke", sliderHandle)
			handleStroke.Thickness = self.Config.UIStrokeThickness
			handleStroke.Color = self.Colors.UIStrokeColor
		end

		local function updateSlider(value, ignoreInput)

			value = math.clamp(value, config.Min, config.Max)
			currentValue = value

			local fillSize = (value - config.Min) / (config.Max - config.Min)

			TS:Create(sliderFill, TweenInfo.new(0.1), { Size = UDim2.new(fillSize, 0, 1, 0) }):Play()
			TS:Create(sliderHandle, TweenInfo.new(0.1), { Position = UDim2.new(fillSize, 0, 0.5, 0) }):Play()

			local displayValue = value

			if config.Round and config.Round > 0 then
				displayValue = tonumber(string.format("%." .. config.Round .. "f", value))
			else
				displayValue = math.floor(value)
			end

			if not ignoreInput then
				valueInput.Text = tostring(displayValue)
			end

			if config.SavingEnabled then
				lib.UserSettings[flag] = displayValue
			end

			if config.Callback then
				config.Callback(displayValue)
			end

		end

		valueInput.FocusLost:Connect(function()

			local val = tonumber(valueInput.Text)

			if not val then
				valueInput.Text = tostring(currentValue)
				return
			end

			updateSlider(val)

		end)

		local function moveFromInput(input)

			local relativeX = math.clamp(
				(input.Position.X - sliderTrack.AbsolutePosition.X) / sliderTrack.AbsoluteSize.X,
				0,
				1
			)

			local value = config.Min + (config.Max - config.Min) * relativeX
			updateSlider(value)

		end

		sliderTrack.InputBegan:Connect(function(input)

			if input.UserInputType == Enum.UserInputType.MouseButton1 or
				input.UserInputType == Enum.UserInputType.Touch then

				dragging = true
				moveFromInput(input)

			end

		end)

		UIS.InputEnded:Connect(function(input)

			if input.UserInputType == Enum.UserInputType.MouseButton1 or
				input.UserInputType == Enum.UserInputType.Touch then

				dragging = false

			end

		end)

		UIS.InputChanged:Connect(function(input)

			if dragging and
				(input.UserInputType == Enum.UserInputType.MouseMovement or
					input.UserInputType == Enum.UserInputType.Touch) then

				moveFromInput(input)

			end

		end)

		if self.UserSettings[flag] ~= nil then
			updateSlider(tonumber(self.UserSettings[flag]))
		else
			updateSlider(currentValue)
		end

		function slider:Set(value)
			updateSlider(value)
		end

		table.insert(self.SettingElements, sliderFrame)
		return slider

	end,

	AddDropdown = function(self, config)
		local dropdownHeight = config.DropDownHeight or 44
		local optionHeight = config.OptionHeight or 30
		local isOpen = false
		local dropdown = {}

		local lib = self

		local dropdownFrame = Instance.new("Frame")
		dropdownFrame.Name = "Dropdown_" .. config.Text
		dropdownFrame.Size = UDim2.new(1, -24, 0, dropdownHeight)
		dropdownFrame.Position = UDim2.new(0, 12, 0, 0)
		dropdownFrame.BackgroundTransparency = 1
		dropdownFrame.ClipsDescendants = true 
		dropdownFrame.LayoutOrder = #self.Elements + 1
		dropdownFrame.Parent = self.ScrollingFrame

		local headerBtn = Instance.new("TextButton")
		headerBtn.Name = "Header"
		headerBtn.Size = UDim2.new(1, 0, 0, dropdownHeight)
		headerBtn.BackgroundColor3 = self.Colors.ButtonColor
		headerBtn.Text = ""
		headerBtn.AutoButtonColor = false
		headerBtn.Parent = dropdownFrame

		local headerCorner = Instance.new("UICorner")
		headerCorner.CornerRadius = UDim.new(0, 6)
		headerCorner.Parent = headerBtn

		if self.Config.UIStrokeEnabled then
			local stroke = Instance.new("UIStroke")
			stroke.Thickness = self.Config.UIStrokeThickness
			stroke.Color = self.Colors.UIStrokeColor
			stroke.Parent = headerBtn
		end

		local titleLabel = Instance.new("TextLabel")
		titleLabel.Name = "Title"
		titleLabel.Size = UDim2.new(1, -40, 0, 20) 
		titleLabel.Position = UDim2.new(0, 10, 0, 5) 
		titleLabel.BackgroundTransparency = 1
		titleLabel.Text = config.Text
		titleLabel.TextColor3 = self.Colors.TitleColor
		titleLabel.TextXAlignment = Enum.TextXAlignment.Left
		titleLabel.Font = self.Config.Font
		titleLabel.TextSize = self.Config.TextSize
		titleLabel.Parent = headerBtn

		local selectedLabel = Instance.new("TextLabel")
		selectedLabel.Name = "Selected"
		selectedLabel.Size = UDim2.new(1, -40, 0, 18)
		selectedLabel.Position = UDim2.new(0, 10, 0, 22)
		selectedLabel.BackgroundTransparency = 1
		selectedLabel.Text = config.Default or "None"
		selectedLabel.TextColor3 = self.Colors.AccentColor 
		selectedLabel.TextXAlignment = Enum.TextXAlignment.Left 
		selectedLabel.Font = self.Config.Font
		selectedLabel.TextSize = self.Config.TextSize - 1 
		selectedLabel.Parent = headerBtn

		local arrow = Instance.new("ImageLabel")
		arrow.Name = "Arrow"
		arrow.Size = UDim2.new(0, 20, 0, 20)
		arrow.Position = UDim2.new(1, -25, 0.5, 0)
		arrow.AnchorPoint = Vector2.new(0, 0.5)
		arrow.BackgroundTransparency = 1
		arrow.Image = "rbxassetid://6034818372" 
		arrow.ImageColor3 = self.Colors.LabelColor
		arrow.Parent = headerBtn

		local optionHolder = Instance.new("Frame")
		optionHolder.Name = "OptionHolder"
		optionHolder.Size = UDim2.new(1, 0, 0, 0) 
		optionHolder.Position = UDim2.new(0, 0, 0, dropdownHeight + 2)
		optionHolder.BackgroundTransparency = 1
		optionHolder.Parent = dropdownFrame

		local optionList = Instance.new("UIListLayout")
		optionList.Padding = UDim.new(0, 2)
		optionList.SortOrder = Enum.SortOrder.LayoutOrder
		optionList.Parent = optionHolder

		local function toggle()
			isOpen = not isOpen

			local count = #config.Options
			local newHeight = isOpen and (dropdownHeight + (count * optionHeight) + (count * 2) + 5) or dropdownHeight

			TS:Create(dropdownFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Size = UDim2.new(1, -24, 0, newHeight)
			}):Play()

			TS:Create(arrow, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Rotation = isOpen and 180 or 0
			}):Play()

			TS:Create(headerBtn, TweenInfo.new(0.3), {
				BackgroundColor3 = isOpen and self.Colors.ButtonHoverColor or self.Colors.ButtonColor
			}):Play()
		end

		local function update(options, clearcurrent)
			for _, child in pairs(optionHolder:GetChildren()) do
				if child:IsA("TextButton") then child:Destroy() end
			end

			local selected = nil

			local currentOptions = options or {}

			if clearcurrent then
				selectedLabel.Text = "None"
				selected = nil
			end

			local flag = config.Flag

			if lib.UserConfig[flag] ~= nil then
				selected = lib.UserConfig[flag]
				selectedLabel.Text = tostring(selected)
			end

			for i, option in ipairs(currentOptions) do
				local optBtn = Instance.new("TextButton")
				optBtn.Name = option
				optBtn.Size = UDim2.new(1, 0, 0, optionHeight)
				optBtn.BackgroundColor3 = lib.Colors.CollapseBtnColor 
				optBtn.Text = option
				optBtn.TextColor3 = lib.Colors.LabelColor
				optBtn.Font = lib.Config.Font
				optBtn.TextSize = lib.Config.TextSize
				optBtn.AutoButtonColor = false
				optBtn.Parent = optionHolder

				local optCorner = Instance.new("UICorner")
				optCorner.CornerRadius = UDim.new(0, 4)
				optCorner.Parent = optBtn

				optBtn.MouseButton1Click:Connect(function()
					selectedLabel.Text = option
					selected = option

					if config.SavingEnabled then
						lib.UserConfig[flag] = selected
					end

					toggle()

					if config.Callback then
						config.Callback(option)
					end
				end)

				optBtn.MouseEnter:Connect(function()
					TS:Create(optBtn, TweenInfo.new(0.2), { BackgroundColor3 = self.Colors.AccentColor, TextColor3 = Color3.new(1,1,1) }):Play()
				end)

				optBtn.MouseLeave:Connect(function()
					TS:Create(optBtn, TweenInfo.new(0.2), { BackgroundColor3 = self.Colors.CollapseBtnColor, TextColor3 = self.Colors.LabelColor }):Play()
				end)
			end
		end

		update(config.Options, true)

		function dropdown:Update(options)
			update(options, true)
		end

		function dropdown:Set(option)
			selectedLabel.Text = tostring(option)

			if config.SavingEnabled and config.Flag then
				lib.UserConfig[config.Flag] = option
			end

			if config.Callback then
				task.spawn(config.Callback, option)
			end

			if isOpen then
				toggle()
			end
		end

		headerBtn.MouseButton1Click:Connect(toggle)

		table.insert(self.Elements, dropdownFrame)
		return dropdown
	end,

	AddMultiDropdown = function(self, config)
		local dropdownHeight = config.DropDownHeight or 44
		local optionHeight = config.OptionHeight or 30
		local isOpen = false
		local dropdown = {}
		local selectedItems = {}

		local lib = self

		local dropdownFrame = Instance.new("Frame")
		dropdownFrame.Name = "MultiDropdown_" .. config.Text
		dropdownFrame.Size = UDim2.new(1, -24, 0, dropdownHeight)
		dropdownFrame.Position = UDim2.new(0, 12, 0, 0)
		dropdownFrame.BackgroundTransparency = 1
		dropdownFrame.ClipsDescendants = true 
		dropdownFrame.LayoutOrder = #self.Elements + 1
		dropdownFrame.Parent = self.ScrollingFrame

		local headerBtn = Instance.new("TextButton")
		headerBtn.Name = "Header"
		headerBtn.Size = UDim2.new(1, 0, 0, dropdownHeight)
		headerBtn.BackgroundColor3 = self.Colors.ButtonColor
		headerBtn.Text = ""
		headerBtn.AutoButtonColor = false
		headerBtn.Parent = dropdownFrame

		local headerCorner = Instance.new("UICorner")
		headerCorner.CornerRadius = UDim.new(0, 6)
		headerCorner.Parent = headerBtn

		if self.Config.UIStrokeEnabled then
			local stroke = Instance.new("UIStroke")
			stroke.Thickness = self.Config.UIStrokeThickness
			stroke.Color = self.Colors.UIStrokeColor
			stroke.Parent = headerBtn
		end

		local titleLabel = Instance.new("TextLabel")
		titleLabel.Name = "Title"
		titleLabel.Size = UDim2.new(1, -40, 0, 20) 
		titleLabel.Position = UDim2.new(0, 10, 0, 5) 
		titleLabel.BackgroundTransparency = 1
		titleLabel.Text = config.Text
		titleLabel.TextColor3 = self.Colors.TitleColor
		titleLabel.TextXAlignment = Enum.TextXAlignment.Left
		titleLabel.Font = self.Config.Font
		titleLabel.TextSize = self.Config.TextSize
		titleLabel.Parent = headerBtn

		local selectedLabel = Instance.new("TextLabel")
		selectedLabel.Name = "Selected"
		selectedLabel.Size = UDim2.new(1, -40, 0, 18)
		selectedLabel.Position = UDim2.new(0, 10, 0, 22)
		selectedLabel.BackgroundTransparency = 1
		selectedLabel.Text = "None"
		selectedLabel.TextColor3 = self.Colors.AccentColor 
		selectedLabel.TextXAlignment = Enum.TextXAlignment.Left 
		selectedLabel.Font = self.Config.Font
		selectedLabel.TextScaled = true
		selectedLabel.Parent = headerBtn

		local sizeConstraint = Instance.new("UITextSizeConstraint")
		sizeConstraint.MaxTextSize = self.Config.TextSize - 1
		sizeConstraint.Parent = selectedLabel

		local arrow = Instance.new("ImageLabel")
		arrow.Name = "Arrow"
		arrow.Size = UDim2.new(0, 20, 0, 20)
		arrow.Position = UDim2.new(1, -25, 0.5, 0)
		arrow.AnchorPoint = Vector2.new(0, 0.5)
		arrow.BackgroundTransparency = 1
		arrow.Image = "rbxassetid://6034818372" 
		arrow.ImageColor3 = self.Colors.LabelColor
		arrow.Parent = headerBtn

		local optionHolder = Instance.new("Frame")
		optionHolder.Name = "OptionHolder"
		optionHolder.Size = UDim2.new(1, 0, 0, 0) 
		optionHolder.Position = UDim2.new(0, 0, 0, dropdownHeight + 2)
		optionHolder.BackgroundTransparency = 1
		optionHolder.Parent = dropdownFrame

		local optionList = Instance.new("UIListLayout")
		optionList.Padding = UDim.new(0, 2)
		optionList.SortOrder = Enum.SortOrder.LayoutOrder
		optionList.Parent = optionHolder

		local function updateHeaderText()
			if #selectedItems == 0 then
				selectedLabel.Text = "None"
			else
				selectedLabel.Text = table.concat(selectedItems, ", ")
			end
		end

		local function toggle()
			isOpen = not isOpen

			local count = #config.Options
			local newHeight = isOpen and (dropdownHeight + (count * optionHeight) + (count * 2) + 5) or dropdownHeight

			TS:Create(dropdownFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Size = UDim2.new(1, -24, 0, newHeight)
			}):Play()

			TS:Create(arrow, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
				Rotation = isOpen and 180 or 0
			}):Play()

			TS:Create(headerBtn, TweenInfo.new(0.3), {
				BackgroundColor3 = isOpen and self.Colors.ButtonHoverColor or self.Colors.ButtonColor
			}):Play()
		end

		local function update(options, clearcurrent)
			for _, child in pairs(optionHolder:GetChildren()) do
				if child:IsA("TextButton") then child:Destroy() end
			end

			local currentOptions = options or {}

			if clearcurrent then
				selectedItems = {}
				updateHeaderText()
			end

			local flag = config.Flag

			if lib.UserConfig[flag] ~= nil and type(lib.UserConfig[flag]) == "table" then
				selectedItems = lib.UserConfig[flag]
				updateHeaderText()
			elseif config.Default then
				if type(config.Default) == "table" then
					selectedItems = config.Default
				else
					selectedItems = {config.Default}
				end
				updateHeaderText()
			end

			for i, option in ipairs(currentOptions) do
				local optBtn = Instance.new("TextButton")
				optBtn.Name = option
				optBtn.Size = UDim2.new(1, 0, 0, optionHeight)

				local isSelected = table.find(selectedItems, option)
				optBtn.BackgroundColor3 = isSelected and self.Colors.AccentColor or lib.Colors.CollapseBtnColor
				optBtn.TextColor3 = isSelected and Color3.new(1,1,1) or lib.Colors.LabelColor

				optBtn.Text = option
				optBtn.Font = lib.Config.Font
				optBtn.TextSize = lib.Config.TextSize
				optBtn.AutoButtonColor = false
				optBtn.Parent = optionHolder

				local optCorner = Instance.new("UICorner")
				optCorner.CornerRadius = UDim.new(0, 4)
				optCorner.Parent = optBtn

				optBtn.MouseButton1Click:Connect(function()
					local index = table.find(selectedItems, option)

					if index then

						table.remove(selectedItems, index)
						TS:Create(optBtn, TweenInfo.new(0.2), { 
							BackgroundColor3 = lib.Colors.CollapseBtnColor, 
							TextColor3 = lib.Colors.LabelColor 
						}):Play()
					else

						table.insert(selectedItems, option)
						TS:Create(optBtn, TweenInfo.new(0.2), { 
							BackgroundColor3 = self.Colors.AccentColor, 
							TextColor3 = Color3.new(1,1,1) 
						}):Play()
					end

					updateHeaderText()

					if config.SavingEnabled then
						lib.UserConfig[flag] = selectedItems
					end

					if config.Callback then
						config.Callback(selectedItems)
					end
				end)
			end
		end

		update(config.Options, true)

		function dropdown:Update(config)
			update(config.Options, true)
		end

		function dropdown:Set(newList)
			selectedItems = type(newList) == "table" and newList or {newList}

			updateHeaderText()

			if config.SavingEnabled and config.Flag then
				lib.UserConfig[config.Flag] = selectedItems
			end

			for _, btn in pairs(optionHolder:GetChildren()) do
				if btn:IsA("TextButton") then
					local isSelected = table.find(selectedItems, btn.Name)

					TS:Create(btn, TweenInfo.new(0.2), {
						BackgroundColor3 = isSelected and self.Colors.AccentColor or lib.Colors.CollapseBtnColor,
						TextColor3 = isSelected and Color3.new(1,1,1) or lib.Colors.LabelColor
					}):Play()
				end
			end

			if config.Callback then
				task.spawn(config.Callback, selectedItems)
			end
		end

		headerBtn.MouseButton1Click:Connect(toggle)

		table.insert(self.Elements, dropdownFrame)
		return dropdown
	end,

	AddColorPicker = function(self, config)
		local TweenService = TS
		local colorpicker = {}
		local lib = self
		local flag = config.Flag
		local saved = self.UserConfig[flag]

		local defaultColor = config.Default or Color3.fromRGB(255, 255, 255)
		if saved and type(saved) == "table" then
			defaultColor = Color3.new(saved[1], saved[2], saved[3])
		end

		local r, g, b = math.floor(defaultColor.R * 255), math.floor(defaultColor.G * 255), math.floor(defaultColor.B * 255)
		local isOpen = false

		local pickerFrame = Instance.new("Frame")
		pickerFrame.Name = "ColorPicker_" .. config.Text
		pickerFrame.Size = UDim2.new(1, -24, 0, 30)
		pickerFrame.BackgroundColor3 = self.Colors.ButtonColor
		pickerFrame.ClipsDescendants = true
		pickerFrame.LayoutOrder = #self.Elements + 1
		pickerFrame.Parent = self.ScrollingFrame

		Instance.new("UICorner", pickerFrame).CornerRadius = UDim.new(0, 4)

		local headerBtn = Instance.new("TextButton")
		headerBtn.Size = UDim2.new(1, 0, 0, 30)
		headerBtn.BackgroundTransparency = 1
		headerBtn.Text = ""
		headerBtn.Parent = pickerFrame

		local title = Instance.new("TextLabel")
		title.Size = UDim2.new(1, -50, 1, 0)
		title.Position = UDim2.new(0, 12, 0, 0)
		title.BackgroundTransparency = 1
		title.Text = config.Text
		title.TextColor3 = self.Colors.TitleColor
		title.Font = self.Config.Font
		title.TextSize = self.Config.TextSize
		title.TextXAlignment = Enum.TextXAlignment.Left
		title.Parent = headerBtn

		local colorDisplay = Instance.new("Frame")
		colorDisplay.Size = UDim2.new(0, 30, 0, 18)
		colorDisplay.Position = UDim2.new(1, -10, 0.5, 0)
		colorDisplay.AnchorPoint = Vector2.new(1, 0.5)
		colorDisplay.BackgroundColor3 = defaultColor
		colorDisplay.Parent = headerBtn
		Instance.new("UICorner", colorDisplay).CornerRadius = UDim.new(0, 4)

		local sliderContainer = Instance.new("Frame")
		sliderContainer.Size = UDim2.new(1, -24, 0, 90)
		sliderContainer.Position = UDim2.new(0, 12, 0, 35)
		sliderContainer.BackgroundTransparency = 1
		sliderContainer.Parent = pickerFrame

		local function createRGBSlider(name, color, pos, startVal)
			local sliderInput = Instance.new("TextBox")
			sliderInput.Name = name .. "_Input"
			sliderInput.Text = startVal
			sliderInput.PlaceholderText = name
			sliderInput.Size = UDim2.new(0, 20, 0, 15)
			sliderInput.Position = UDim2.new(0, 15, 0, pos)
			sliderInput.BackgroundTransparency = 0
			sliderInput.BackgroundColor3 = self.Colors.SectionColor
			sliderInput.TextColor3 = self.Colors.TitleColor
			sliderInput.TextSize = 10
			sliderInput.Font = self.Config.Font
			sliderInput.TextXAlignment = Enum.TextXAlignment.Center
			sliderInput.ClearTextOnFocus = false
			sliderInput.Parent = sliderContainer

			local nameLabel = Instance.new("TextLabel")
			nameLabel.Text = name .. ":"
			nameLabel.Size = UDim2.new(0, 15, 0, 15)
			nameLabel.Position = UDim2.new(0, 0, 0, pos)
			nameLabel.BackgroundTransparency = 1
			nameLabel.TextColor3 = self.Colors.AccentColor
			nameLabel.TextSize = 12
			nameLabel.Font = self.Config.Font
			nameLabel.Parent = sliderContainer

			if name == "R" then
				nameLabel.TextColor3 = Color3.fromRGB(255, 0, 0)
			elseif name == "G" then
				nameLabel.TextColor3 = Color3.fromRGB(0, 255, 0)
			elseif name == "B" then
				nameLabel.TextColor3 = Color3.fromRGB(0, 0, 255)
			end

			local namecorner = Instance.new("UICorner", sliderInput)
			namecorner.CornerRadius = UDim.new(0, 4)

			local track = Instance.new("TextButton")
			track.Size = UDim2.new(1, -15, 0, 4)
			track.Position = UDim2.new(0, 15, 0, pos + 18)
			track.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
			track.BorderSizePixel = 0
			track.Text = ""
			track.Parent = sliderContainer
			Instance.new("UICorner", track)

			local fill = Instance.new("Frame")
			fill.Size = UDim2.new(startVal/255, 0, 1, 0)
			fill.BackgroundColor3 = color
			fill.BorderSizePixel = 0
			fill.Parent = track
			Instance.new("UICorner", fill)

			local function updateEverything()
				local newColor = Color3.fromRGB(r, g, b)
				colorDisplay.BackgroundColor3 = newColor

				if config.SavingEnabled then
					lib.UserConfig[flag] = {newColor.R, newColor.G, newColor.B}
				end
				if config.Callback then config.Callback(newColor) end
			end

			local dragging = false
			local function updateFromSlider(input)
				local ratio = math.clamp((input.Position.X - track.AbsolutePosition.X) / track.AbsoluteSize.X, 0, 1)
				local val = math.floor(ratio * 255)

				fill.Size = UDim2.new(ratio, 0, 1, 0)
				sliderInput.Text = tostring(val)

				if name == "R" then r = val
				elseif name == "G" then g = val
				elseif name == "B" then b = val end

				updateEverything()
			end

			function colorpicker:Set(color)
				r, g, b = color.R, color.G, color.B
			end

			sliderInput.FocusLost:Connect(function()
				local val = tonumber(sliderInput.Text)
				if not val then
					sliderInput.Text = (name == "R" and r or name == "G" and g or b)
					return
				end

				val = math.clamp(math.floor(val), 0, 255)
				sliderInput.Text = tostring(val)
				fill.Size = UDim2.new(val/255, 0, 1, 0)

				if name == "R" then r = val
				elseif name == "G" then g = val
				elseif name == "B" then b = val end

				updateEverything()
			end)

			track.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
					dragging = true
					updateFromSlider(input)
				end
			end)

			UIS.InputEnded:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
					dragging = false
				end
			end)

			UIS.InputChanged:Connect(function(input)
				if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
					updateFromSlider(input)
				end
			end)
		end

		createRGBSlider("R", Color3.fromRGB(255, 70, 70), 0, r)
		createRGBSlider("G", Color3.fromRGB(70, 255, 70), 30, g)
		createRGBSlider("B", Color3.fromRGB(70, 70, 255), 60, b)

		headerBtn.MouseButton1Click:Connect(function()
			isOpen = not isOpen
			TweenService:Create(pickerFrame, TweenInfo.new(0.3), {
				Size = UDim2.new(1, -24, 0, isOpen and 135 or 30)
			}):Play()
		end)

		table.insert(self.Elements, pickerFrame)
		return colorpicker
	end,

	AddKeybind = function(self, config)
		local keybind = {}
		local UIS = game:GetService("UserInputService")
		local currentKey = config.Default or Enum.KeyCode.F
		local binding = false
		local flag = config.Flag

		local keybindFrame = Instance.new("Frame")
		keybindFrame.Name = "Keybind_" .. config.Text
		keybindFrame.Size = UDim2.new(1, -24, 0, 30)
		keybindFrame.BackgroundColor3 = self.Colors.ButtonColor
		keybindFrame.BorderSizePixel = 0
		keybindFrame.LayoutOrder = #self.Elements + 1
		keybindFrame.Parent = self.ScrollingFrame

		local corner = Instance.new("UICorner")
		corner.CornerRadius = UDim.new(0, 4)
		corner.Parent = keybindFrame

		local title = Instance.new("TextLabel")
		title.Name = "Title"
		title.Size = UDim2.new(1, -12, 1, 0)
		title.Position = UDim2.new(0, 12, 0, 0)
		title.BackgroundTransparency = 1
		title.Text = config.Text
		title.TextColor3 = self.Colors.TitleColor
		title.Font = self.Config.Font
		title.TextSize = self.Config.TextSize
		title.TextXAlignment = Enum.TextXAlignment.Left
		title.Parent = keybindFrame

		local bindButton = Instance.new("TextButton")
		bindButton.Name = "BindDisplay"
		bindButton.Size = UDim2.new(0, 30, 0, 20)
		bindButton.Position = UDim2.new(1, -10, 0.5, 0)
		bindButton.AnchorPoint = Vector2.new(1, 0.5)
		bindButton.BackgroundColor3 = self.Colors.CollapseBtnColor
		bindButton.Text = currentKey.Name
		bindButton.TextColor3 = self.Colors.AccentColor
		bindButton.Font = Enum.Font.GothamBold
		bindButton.TextSize = 12
		bindButton.AutoButtonColor = false
		bindButton.Parent = keybindFrame

		local bindCorner = Instance.new("UICorner")
		bindCorner.CornerRadius = UDim.new(0, 4)
		bindCorner.Parent = bindButton

		if self.UserConfig[flag] ~= nil then
			currentKey = self.UserConfig[flag]
		end

		bindButton.MouseButton1Click:Connect(function()
			if binding then return end

			binding = true
			bindButton.Text = "..."
			TS:Create(bindButton, TweenInfo.new(0.2), {BackgroundColor3 = self.Colors.AccentColor, TextColor3 = Color3.new(1,1,1)}):Play()
		end)

		UIS.InputBegan:Connect(function(input, gpe)
			if gpe then return end

			if binding then
				if input.UserInputType == Enum.UserInputType.Keyboard then
					currentKey = input.KeyCode
					bindButton.Text = currentKey.Name
					binding = false

					TS:Create(bindButton, TweenInfo.new(0.2), {BackgroundColor3 = self.Colors.CollapseBtnColor, TextColor3 = self.Colors.AccentColor}):Play()

					if config.SavingEnabled then
						self.UserConfig[flag] = currentKey
					end

					if config.ChangedCallback then
						config.ChangedCallback(currentKey)
					end
				end
			else
				if input.KeyCode == currentKey then
					if config.Callback then
						config.Callback()
					end
				end
			end
		end)

		function keybind:Set(key)
			currentKey = key
			if config.SavingEnabled then
				self.UserConfig[flag] = currentKey
			end

			if config.ChangedCallback then
				config.ChangedCallback(currentKey)
			end
		end

		table.insert(self.Elements, keybindFrame)
		return keybind
	end,

	AddSettingKeybind = function(self, config)
		local keybind = {}
		local UIS = game:GetService("UserInputService")
		local currentKey = config.Default or Enum.KeyCode.F2
		local binding = false
		local flag = config.Flag

		local keybindFrame = Instance.new("Frame")
		keybindFrame.Name = "Keybind_" .. config.Text
		keybindFrame.Size = UDim2.new(1, -5, 0, 30)
		keybindFrame.BackgroundColor3 = self.Colors.ButtonColor
		keybindFrame.BorderSizePixel = 0
		keybindFrame.LayoutOrder = #self.Elements + 1
		keybindFrame.Parent = self.SettingScrollingFrame

		local corner = Instance.new("UICorner")
		corner.CornerRadius = UDim.new(0, 4)
		corner.Parent = keybindFrame

		local title = Instance.new("TextLabel")
		title.Name = "Title"
		title.Size = UDim2.new(1, -12, 1, 0)
		title.Position = UDim2.new(0, 12, 0, 0)
		title.BackgroundTransparency = 1
		title.Text = config.Text
		title.TextColor3 = self.Colors.TitleColor
		title.Font = self.Config.Font
		title.TextSize = self.Config.TextSize
		title.TextXAlignment = Enum.TextXAlignment.Left
		title.Parent = keybindFrame

		local bindButton = Instance.new("TextButton")
		bindButton.Name = "BindDisplay"
		bindButton.Size = UDim2.new(0, 30, 0, 20)
		bindButton.Position = UDim2.new(1, -10, 0.5, 0)
		bindButton.AnchorPoint = Vector2.new(1, 0.5)
		bindButton.BackgroundColor3 = self.Colors.CollapseBtnColor
		bindButton.Text = currentKey.Name
		bindButton.TextColor3 = self.Colors.AccentColor
		bindButton.Font = Enum.Font.GothamBold
		bindButton.TextSize = 12
		bindButton.AutoButtonColor = false
		bindButton.Parent = keybindFrame

		local bindCorner = Instance.new("UICorner")
		bindCorner.CornerRadius = UDim.new(0, 4)
		bindCorner.Parent = bindButton

		if self.UserConfig[flag] ~= nil then
			currentKey = self.UserConfig[flag]
		end

		bindButton.MouseButton1Click:Connect(function()
			if binding then return end

			binding = true
			bindButton.Text = "..."
			TS:Create(bindButton, TweenInfo.new(0.2), {BackgroundColor3 = self.Colors.AccentColor, TextColor3 = Color3.new(1,1,1)}):Play()
		end)

		UIS.InputBegan:Connect(function(input, gpe)
			if gpe then return end

			if binding then
				if input.UserInputType == Enum.UserInputType.Keyboard then
					currentKey = input.KeyCode
					bindButton.Text = currentKey.Name
					binding = false

					TS:Create(bindButton, TweenInfo.new(0.2), {BackgroundColor3 = self.Colors.CollapseBtnColor, TextColor3 = self.Colors.AccentColor}):Play()

					if config.SavingEnabled then
						self.UserConfig[flag] = currentKey
					end

					if config.ChangedCallback then
						config.ChangedCallback(currentKey)
					end
				end
			else
				if input.KeyCode == currentKey then
					if config.Callback then
						config.Callback()
					end
				end
			end
		end)

		function keybind:Set(key)
			currentKey = key
			if config.SavingEnabled then
				self.UserConfig[flag] = currentKey
			end

			if config.ChangedCallback then
				config.ChangedCallback(currentKey)
			end
		end

		table.insert(self.Elements, keybindFrame)
		return keybind
	end,

	AddTextBox = function(self, config)
		local flag = config.Flag
		local lib = self
		local textBoxT = {}
		local textBoxFrame = Instance.new("Frame")
		textBoxFrame.Name = "TextBox_" .. config.Text
		textBoxFrame.Size = UDim2.new(1, -24, 0, 50)
		textBoxFrame.Position = UDim2.new(0, 12, 0, 0)
		textBoxFrame.BackgroundTransparency = 1
		textBoxFrame.LayoutOrder = #self.Elements + 1
		textBoxFrame.Parent = self.ScrollingFrame

		local textBoxLabel = Instance.new("TextLabel")
		textBoxLabel.Name = "TextLabel"
		textBoxLabel.Size = UDim2.new(1, 0, 0.4, 0)
		textBoxLabel.Position = UDim2.new(0, 0, 0, 0)
		textBoxLabel.BackgroundTransparency = 1
		textBoxLabel.Text = config.Text
		textBoxLabel.TextColor3 = self.Colors.TitleColor
		textBoxLabel.TextXAlignment = Enum.TextXAlignment.Left
		textBoxLabel.Font = self.Config.Font
		textBoxLabel.TextSize = self.Config.TextSize
		textBoxLabel.Parent = textBoxFrame

		local textBox = Instance.new("TextBox")
		textBox.Name = "TextBox"
		textBox.Size = UDim2.new(1, 0, 0.6, 0)
		textBox.Position = UDim2.new(0, 0, 0.4, 0)
		textBox.BackgroundColor3 = self.Colors.TextBoxColor
		textBox.BorderSizePixel = 0
		textBox.Text = config.Default or ""
		textBox.TextColor3 = self.Colors.TitleColor
		textBox.Font = self.Config.Font
		textBox.TextSize = self.Config.TextSize
		textBox.PlaceholderText = config.Placeholder or ""
		textBox.PlaceholderColor3 = Color3.new(0.7, 0.7, 0.7)
		textBox.Parent = textBoxFrame

		local corner = Instance.new("UICorner")
		corner.CornerRadius = UDim.new(0, 4)
		corner.Parent = textBox

		if self.Config.UIStrokeEnabled then
			local textBoxStroke = Instance.new("UIStroke")
			textBoxStroke.Thickness = self.Config.UIStrokeThickness
			textBoxStroke.Color = self.Colors.UIStrokeColor
			textBoxStroke.Parent = textBox
		end

		function textBoxT:Set(s)
			textBox.Text = tostring(s) or "Type can not be converted into a string"

			if config.Callback then
				config.Callback(textBox.Text)
			end
		end

		if config.SavingEnabled then
			if self.UserConfig[flag] ~= nil then
				textBoxT:Set(tostring(self.UserConfig[flag]))
			end
		end

		if config.Callback then
			textBox.FocusLost:Connect(function(enterPressed)
				if not enterPressed and config.RequireEnter then return end
				config.Callback(textBox.Text)
			end)
		end

		table.insert(self.Elements, textBoxFrame)
		return textBoxT
	end,
	
	AddSettingTextBox = function(self, config)

		local flag = config.Flag
		local lib = self
		local textBoxT = {}

		local textBoxFrame = Instance.new("Frame")
		textBoxFrame.Name = "TextBox_" .. config.Text
		textBoxFrame.Size = UDim2.new(1, -12, 0, 50)
		textBoxFrame.Position = UDim2.new(0, 12, 0, 0)
		textBoxFrame.BackgroundTransparency = 1
		textBoxFrame.LayoutOrder = #self.Elements + 1
		textBoxFrame.Parent = self.ScrollingFrame

		local textBoxLabel = Instance.new("TextLabel")
		textBoxLabel.Name = "TextLabel"
		textBoxLabel.Size = UDim2.new(1, 0, 0.4, 0)
		textBoxLabel.Position = UDim2.new(0, 0, 0, 0)
		textBoxLabel.BackgroundTransparency = 1
		textBoxLabel.Text = config.Text
		textBoxLabel.TextColor3 = self.Colors.TitleColor
		textBoxLabel.TextXAlignment = Enum.TextXAlignment.Left
		textBoxLabel.Font = self.Config.Font
		textBoxLabel.TextSize = self.Config.TextSize
		textBoxLabel.Parent = textBoxFrame

		local textBox = Instance.new("TextBox")
		textBox.Name = "TextBox"
		textBox.Size = UDim2.new(1, 0, 0.6, 0)
		textBox.Position = UDim2.new(0, 0, 0.4, 0)
		textBox.BackgroundColor3 = self.Colors.TextBoxColor
		textBox.BorderSizePixel = 0
		textBox.Text = config.Default or ""
		textBox.TextColor3 = self.Colors.TitleColor
		textBox.Font = self.Config.Font
		textBox.TextSize = self.Config.TextSize
		textBox.PlaceholderText = config.Placeholder or ""
		textBox.PlaceholderColor3 = Color3.new(0.7, 0.7, 0.7)
		textBox.Parent = textBoxFrame

		local corner = Instance.new("UICorner")
		corner.CornerRadius = UDim.new(0, 4)
		corner.Parent = textBox

		if self.Config.UIStrokeEnabled then
			local textBoxStroke = Instance.new("UIStroke")
			textBoxStroke.Thickness = self.Config.UIStrokeThickness
			textBoxStroke.Color = self.Colors.UIStrokeColor
			textBoxStroke.Parent = textBox
		end

		function textBoxT:Set(s)

			textBox.Text = tostring(s) or "Type can not be converted into a string"

			if config.Callback then
				config.Callback(textBox.Text)
			end

		end

		if config.SavingEnabled then

			if self.UserSettings[flag] ~= nil then
				textBoxT:Set(tostring(self.UserSettings[flag]))
			end

		end

		if config.Callback then

			textBox.FocusLost:Connect(function(enterPressed)

				if not enterPressed and config.RequireEnter then
					return
				end

				config.Callback(textBox.Text)

			end)

		end

		table.insert(self.Elements, textBoxFrame)
		return textBoxT

	end,

	AddSeparator = function(self)
		local separator = Instance.new("Frame")
		separator.Name = "Separator"
		separator.Size = UDim2.new(1, -24, 0, 1)
		separator.Position = UDim2.new(0, 12, 0, 0)
		separator.BackgroundColor3 = self.Colors.SeparatorColor
		separator.BorderSizePixel = 0
		separator.LayoutOrder = #self.Elements + 1
		separator.Parent = self.ScrollingFrame

		table.insert(self.Elements, separator)
		return separator
	end,

	AddSlider = function(self, config)
		local lib = self
		local slider = {}

		local dragging = false
		local currentValue = config.Default or config.Min
		local flag = config.Flag

		local sliderFrame = Instance.new("Frame")
		sliderFrame.Name = "Slider_" .. config.Text
		sliderFrame.Size = UDim2.new(1, -24, 0, 50)
		sliderFrame.BackgroundTransparency = 1
		sliderFrame.LayoutOrder = #self.Elements + 1
		sliderFrame.Parent = self.ScrollingFrame

		local title = Instance.new("TextLabel")
		title.Name = "Title"
		title.Size = UDim2.new(1, -60, 0, 20)
		title.Position = UDim2.new(0, 0, 0, 0)
		title.BackgroundTransparency = 1
		title.Text = config.Text
		title.TextColor3 = self.Colors.TitleColor
		title.Font = self.Config.Font
		title.TextSize = self.Config.TextSize
		title.TextXAlignment = Enum.TextXAlignment.Left
		title.Parent = sliderFrame

		local valueInput = Instance.new("TextBox")
		valueInput.Name = "ValueInput"
		valueInput.Size = UDim2.new(0, 50, 0, 20)
		valueInput.Position = UDim2.new(1, 0, 0, 0)
		valueInput.AnchorPoint = Vector2.new(1, 0)
		valueInput.BackgroundTransparency = 0.1
		valueInput.BackgroundColor3 = self.Colors.ButtonColor
		valueInput.Text = tostring(currentValue)
		valueInput.TextColor3 = self.Colors.AccentColor
		valueInput.Font = self.Config.Font
		valueInput.TextSize = self.Config.TextSize - 2
		valueInput.ClearTextOnFocus = false
		valueInput.Parent = sliderFrame
		Instance.new("UICorner", valueInput).CornerRadius = UDim.new(0, 4)

		local sliderTrack = Instance.new("TextButton")
		sliderTrack.Name = "Track"
		sliderTrack.Text = ""
		sliderTrack.Size = UDim2.new(1, 0, 0, 6)
		sliderTrack.Position = UDim2.new(0, 0, 0, 32)
		sliderTrack.BackgroundColor3 = self.Colors.SliderColor
		sliderTrack.BorderSizePixel = 0
		sliderTrack.AutoButtonColor = false
		sliderTrack.Parent = sliderFrame
		Instance.new("UICorner", sliderTrack).CornerRadius = UDim.new(1, 0)

		local sliderFill = Instance.new("Frame")
		sliderFill.Name = "Fill"
		sliderFill.Size = UDim2.new(0, 0, 1, 0)
		sliderFill.BackgroundColor3 = self.Colors.AccentColor
		sliderFill.BorderSizePixel = 0
		sliderFill.Parent = sliderTrack
		Instance.new("UICorner", sliderFill).CornerRadius = UDim.new(1, 0)

		local sliderHandle = Instance.new("Frame")
		sliderHandle.Name = "Handle"
		sliderHandle.Size = UDim2.new(0, 14, 0, 14)
		sliderHandle.AnchorPoint = Vector2.new(0.5, 0.5)
		sliderHandle.Position = UDim2.new(0, 0, 0.5, 0)
		sliderHandle.BackgroundColor3 = self.Colors.SliderHandleColor
		sliderHandle.Parent = sliderTrack
		Instance.new("UICorner", sliderHandle).CornerRadius = UDim.new(1, 0)

		if self.Config.UIStrokeEnabled then
			local handleStroke = Instance.new("UIStroke", sliderHandle)
			handleStroke.Thickness = self.Config.UIStrokeThickness
			handleStroke.Color = self.Colors.UIStrokeColor
		end

		local function updateSlider(value, ignoreInput)
			value = math.clamp(value, config.Min, config.Max)
			currentValue = value

			local fillSize = (value - config.Min) / (config.Max - config.Min)

			TS:Create(sliderFill, TweenInfo.new(0.1), {Size = UDim2.new(fillSize, 0, 1, 0)}):Play()
			TS:Create(sliderHandle, TweenInfo.new(0.1), {Position = UDim2.new(fillSize, 0, 0.5, 0)}):Play()

			local displayValue = value
			if config.Round and config.Round > 0 then
				displayValue = tonumber(string.format("%."..config.Round.."f", value))
			else
				displayValue = math.floor(value)
			end

			if not ignoreInput then
				valueInput.Text = tostring(displayValue)
			end

			if config.SavingEnabled then
				lib.UserConfig[flag] = displayValue
			end

			if config.Callback then
				config.Callback(displayValue)
			end
		end

		valueInput.FocusLost:Connect(function()
			local val = tonumber(valueInput.Text)
			if not val then
				valueInput.Text = tostring(currentValue)
				return
			end
			updateSlider(val)
		end)

		local function moveFromInput(input)
			local relativeX = math.clamp((input.Position.X - sliderTrack.AbsolutePosition.X) / sliderTrack.AbsoluteSize.X, 0, 1)
			local value = config.Min + (config.Max - config.Min) * relativeX
			updateSlider(value)
		end

		sliderTrack.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				dragging = true
				moveFromInput(input)
			end
		end)

		UIS.InputEnded:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				dragging = false
			end
		end)

		UIS.InputChanged:Connect(function(input)
			if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
				moveFromInput(input)
			end
		end)

		if self.UserConfig[flag] ~= nil then
			updateSlider(tonumber(self.UserConfig[flag]))
		else
			updateSlider(currentValue)
		end

		function slider:Set(value) updateSlider(value) end

		table.insert(self.Elements, sliderFrame)
		return slider
	end,

	Notify = function(self, config)   
		local accentColor = config.AccentColor or Color3.fromRGB(0, 200, 255)

		local sound = Instance.new("Sound")
		sound.SoundId = self.Config.NotifcationSound
		sound.Parent = game:GetService("SoundService")
		sound.Volume = 2
		sound:Play()
		game:GetService("Debris"):AddItem(sound, 2)

		local frame = Instance.new("CanvasGroup")
		frame.Name = "Notification"
		frame.Parent = self.ScreenGui
		frame.Size = UDim2.new(0, 200, 0, 80)
		frame.Position = config.StartPosition
		frame.BackgroundColor3 = Color3.fromRGB(100, 100, 100) 
		frame.GroupTransparency = 1
		frame.BorderSizePixel = 0

		local bgGradient = Instance.new("UIGradient")
		bgGradient.Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0, Color3.fromRGB(80, 80, 90)), -- Bright top-left
			ColorSequenceKeypoint.new(1, Color3.fromRGB(40, 40, 45))  -- Darker bottom-right
		})
		bgGradient.Rotation = 45
		bgGradient.Parent = frame

		local uicorner = Instance.new("UICorner")
		uicorner.CornerRadius = UDim.new(0, 12)
		uicorner.Parent = frame

		local uistroke = Instance.new("UIStroke")
		uistroke.Color = accentColor
		uistroke.Thickness = 2
		uistroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
		uistroke.Transparency = 0.2
		uistroke.Parent = frame

		local accent = Instance.new("Frame")
		accent.Size = UDim2.new(0, 5, 1, 0)
		accent.BackgroundColor3 = accentColor
		accent.BorderSizePixel = 0
		accent.Parent = frame

		local title = Instance.new("TextLabel")
		title.Parent = frame
		title.Size = UDim2.new(1, -70, 0, 35)
		title.Position = UDim2.new(0, 20, 0, 10)
		title.BackgroundTransparency = 1
		title.Text = config.Title
		title.TextColor3 = Color3.fromRGB(255, 255, 255)
		title.TextSize = 18
		title.Font = Enum.Font.GothamBold
		title.TextXAlignment = Enum.TextXAlignment.Left

		local msg = Instance.new("TextLabel")
		msg.Parent = frame
		msg.Size = UDim2.new(1, -40, 0, 40)
		msg.Position = UDim2.new(0, 20, 0, 40)
		msg.BackgroundTransparency = 1
		msg.Text = config.Text
		msg.TextColor3 = Color3.fromRGB(245, 245, 245)
		msg.TextSize = 14
		msg.Font = Enum.Font.GothamBold
		msg.TextWrapped = true
		msg.TextXAlignment = Enum.TextXAlignment.Left
		msg.TextYAlignment = Enum.TextYAlignment.Top

		local closeBtn = Instance.new("TextButton")
		closeBtn.Parent = frame
		closeBtn.Size = UDim2.new(0, 30, 0, 30)
		closeBtn.Position = UDim2.new(1, -35, 0, 10)
		closeBtn.BackgroundTransparency = 1
		closeBtn.Text = "×"
		closeBtn.TextColor3 = Color3.fromRGB(255, 0, 0)
		closeBtn.TextSize = 20
		closeBtn.Font = Enum.Font.GothamBold

		local progressBarBackground = Instance.new("Frame")
		progressBarBackground.Name = "ProgressBarBackground"
		progressBarBackground.Size = UDim2.new(1, 0, 0, 10)
		progressBarBackground.Position = UDim2.new(0, 0, 1, -10)
		progressBarBackground.BackgroundColor3 = Color3.fromRGB(0,0,0)
		progressBarBackground.BackgroundTransparency = 0.7
		progressBarBackground.BorderSizePixel = 0
		progressBarBackground.Parent = frame

		local progressBar = Instance.new("Frame")
		progressBar.Name = "ProgressBar"
		progressBar.Size = UDim2.new(1, 0, 1, 0)
		progressBar.BackgroundColor3 = Color3.fromRGB(255,255,255)
		progressBar.BorderSizePixel = 0
		progressBar.Parent = progressBarBackground

		local tweenInfo = TweenInfo.new(0.6, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out)

		local function dismiss()
			TS:Create(frame, tweenInfo, {Position = config.StartPosition, GroupTransparency = 1}):Play()
			task.wait(0.6)
			frame:Destroy()
		end

		closeBtn.MouseButton1Click:Connect(dismiss)

		TS:Create(frame, tweenInfo, {Position = config.EndPosition, GroupTransparency = 0}):Play()

		TS:Create(progressBar, TweenInfo.new(config.Duration or 5, Enum.EasingStyle.Linear), {Size = UDim2.new(0, 0, 1, 0)}):Play()

		task.delay(config.Duration or 5, function()
			if frame and frame.Parent then dismiss() end
		end)
	end,

	GetVersion = function()
		return "SBEUILib 2.21"
	end,

	PromptDestroy = function(self)
		local Dimmer = Instance.new("Frame")
		Dimmer.Name = "PromptDimmer"
		Dimmer.Size = UDim2.new(1, 0, 1, 0)
		Dimmer.BackgroundColor3 = Color3.new(0, 0, 0)
		Dimmer.BackgroundTransparency = 1
		Dimmer.ZIndex = 100 
		Dimmer.Parent = self.ScreenGui

		local Modal = Instance.new("CanvasGroup")
		Modal.Name = "DestroyModal"
		Modal.Size = UDim2.new(0, 260, 0, 140)
		Modal.Position = UDim2.new(0.5, 0, 0.5, 0)
		Modal.AnchorPoint = Vector2.new(0.5, 0.5)
		Modal.BackgroundColor3 = self.Colors.MainFrameColor
		Modal.GroupTransparency = 1
		Modal.ZIndex = 101
		Modal.Parent = self.ScreenGui

		Instance.new("UICorner", Modal).CornerRadius = UDim.new(0, self.Config.CornerRadius)
		local Stroke = Instance.new("UIStroke", Modal)
		Stroke.Color = self.Colors.UIStrokeColor
		Stroke.Thickness = self.Config.UIStrokeThickness

		local WarningIcon = Instance.new("TextLabel", Modal)
		WarningIcon.Size = UDim2.new(1, 0, 0, 40)
		WarningIcon.Position = UDim2.new(0, 0, 0, 10)
		WarningIcon.BackgroundTransparency = 1
		WarningIcon.Text = "⚠️"
		WarningIcon.TextSize = 24

		local Title = Instance.new("TextLabel", Modal)
		Title.Size = UDim2.new(1, 0, 0, 20)
		Title.Position = UDim2.new(0, 0, 0, 50)
		Title.BackgroundTransparency = 1
		Title.Text = "Are you sure?"
		Title.TextColor3 = Color3.new(1, 1, 1)
		Title.Font = Enum.Font.GothamBold
		Title.TextSize = 16

		local SubTitle = Instance.new("TextLabel", Modal)
		SubTitle.Size = UDim2.new(1, 0, 0, 20)
		SubTitle.Position = UDim2.new(0, 0, 0, 70)
		SubTitle.BackgroundTransparency = 1
		SubTitle.Text = "This will close the entire script."
		SubTitle.TextColor3 = self.Colors.LabelColor
		SubTitle.Font = Enum.Font.Gotham
		SubTitle.TextSize = 12

		local BtnContainer = Instance.new("Frame", Modal)
		BtnContainer.Size = UDim2.new(1, -30, 0, 32)
		BtnContainer.Position = UDim2.new(0, 15, 1, -45)
		BtnContainer.BackgroundTransparency = 1

		local Layout = Instance.new("UIListLayout", BtnContainer)
		Layout.FillDirection = Enum.FillDirection.Horizontal
		Layout.HorizontalAlignment = Enum.HorizontalAlignment.Center
		Layout.Padding = UDim.new(0, 10)

		local function CreatePromptBtn(text, color)
			local b = Instance.new("TextButton", BtnContainer)
			b.Size = UDim2.new(0.48, 0, 1, 0)
			b.BackgroundColor3 = color
			b.Text = text
			b.TextColor3 = Color3.new(1, 1, 1)
			b.Font = Enum.Font.GothamBold
			b.TextSize = 14
			b.AutoButtonColor = false
			Instance.new("UICorner", b).CornerRadius = UDim.new(0, 6)

			b.MouseEnter:Connect(function()
				TS:Create(b, TweenInfo.new(0.2), {BackgroundColor3 = color:Lerp(Color3.new(1,1,1), 0.1)}):Play()
			end)
			b.MouseLeave:Connect(function()
				TS:Create(b, TweenInfo.new(0.2), {BackgroundColor3 = color}):Play()
			end)

			return b
		end

		local CancelBtn = CreatePromptBtn("Cancel", Color3.fromRGB(60, 60, 60))
		local ConfirmBtn = CreatePromptBtn("Close", Color3.fromRGB(180, 40, 40))

		TS:Create(Dimmer, TweenInfo.new(0.4), {BackgroundTransparency = 0.5}):Play()

		Modal.Size = UDim2.new(0, 200, 0, 100) 

		TS:Create(Modal, TweenInfo.new(0.4, Enum.EasingStyle.Back), {
			Size = UDim2.new(0, 260, 0, 140),
			GroupTransparency = 0
		}):Play()

		local function ClosePrompt()
			TS:Create(Dimmer, TweenInfo.new(0.3), {BackgroundTransparency = 1}):Play()
			local t = TS:Create(Modal, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {
				Size = UDim2.new(0, 200, 0, 100),
				GroupTransparency = 1
			})
			t:Play()
			t.Completed:Connect(function()
				Dimmer:Destroy()
				Modal:Destroy()
			end)
		end

		CancelBtn.MouseButton1Click:Connect(ClosePrompt)

		ConfirmBtn.MouseButton1Click:Connect(function()
			ConfirmBtn.Text = "Closing..."
			task.wait(0.2)
			self:Destroy()
		end)
	end,

	SaveConfigurations = function(self)
		local path = self.UserConfigFile
		local path2 = self.UserSettingFile
		
		utils:WriteFile({Path = path, Data = self.UserConfig, IsJSON = true})
		utils:WriteFile({Path = path2, Data = self.UserSettings, IsJSON = true})
	end,

	LoadConfigurations = function(self)
		local filename = self.UserConfigFile
		local filename2 = self.UserSettingFile

		if isfile(filename) then
			local data = utils:ReadFile({Path = filename, IsJSON = true})

			if data then
				self.UserConfig = data
			end
		end
		
		if isfile(filename2) then
			local data = utils:ReadFile({Path = filename2, IsJSON = true})

			if data then
				self.UserSettings = data
			end
		end
	end,

	Destroy = function(self)
		self:SaveConfigurations()

		if self.ScreenGui then
			self.ScreenGui:Destroy()
		end
		self = nil
	end
}

--[[
	well thats it folks
	this took so long to make
	hope u like it!
	
	- sugablaz
]]

return UILibrary
